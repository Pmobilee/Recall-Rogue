#!/usr/bin/env python3
"""
Merge batch 6 knowledge facts into seed files and rebuild the database.
"""
import json
import re
import os
...
DOMAIN_MAP = {
    "toolu_01QEEbBn9GD3wGWvYQbtMtuR": "animals_wildlife",
    "toolu_01Eft3sKuE2yEERq5CSej93u": "art_architecture",
    "toolu_01MzCaNRRMNr49eCQemmo6Vb": "food_cuisine",
    "toolu_01MuzADD1TouvZfuX33xvbJR": "general_knowledge",
    "toolu_016sM42vEy2aRMsavEnDMJ9F": "geography",
    "toolu_01DZPPfxpZsRd2g5aiLcSu4S": "history",
    "toolu_01F3qsn6o2VfzcwLLa5gtbUf": "human_body_health",
    "toolu_01AG57AN6dcSpYWnDczBe4Y4": "mythology_folklore",
    "toolu_01EdtcyziuLvTJXqzUn6oBfJ": "natural_sciences",
    "toolu_012kj8NefjD3hrCof6XJNgev": "space_astronomy",
}
...
# Fields to strip but only if they are arrays (category as array)
# We handle category separately


def extract_facts_from_text(text: str) -> list:
    """Extract the facts JSON array from potentially markdown-wrapped text."""
    # Find all positions where a JSON array starting with [{ or [ { appears
    # We want the first array that looks like a facts array (has "id" field)

    # Strategy: find all [ positions that are followed (possibly with whitespace) by {
    ...
    for match in matches:
        ...
        try:
            ...
            # Validate: must be a list of dicts with 'id' field
...
def fix_fact(fact: dict, domain: str) -> dict:
    ...
    for v in variants:
        if isinstance(v, dict):
            fv = dict(v)
            # Rename correctAnswer -> answer if present (variants use 'answer')
            if "correctAnswer" in fv and "answer" not in fv:
                fv["answer"] = fv.pop("correctAnswer")
            # Fix distractor objects in variants
    ...
    # category field (if it's an array) can also be treated as tags — skip it
...
def main():
    print("=== Batch 6 Knowledge Facts Merge ===\n")
