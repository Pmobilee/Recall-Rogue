...
describe('getEnrageBonus', () => {
  // Shallow (floors 1-6): enrage starts turn 8 (reduced from 12 on 2026-04-09), phase1_bonus=2, phase2_bonus=4, phase1_duration=3
  it('returns 0 before enrage starts on shallow floors', () => {
    expect(getEnrageBonus(7, 3, 1.0)).toBe(0);
  });
  it('returns phase 1 bonus on shallow floors', () => {
    expect(getEnrageBonus(8, 3, 1.0)).toBe(2);   // turn 8: 1 enrage turn * 2
    expect(getEnrageBonus(9, 3, 1.0)).toBe(4);   // turn 9: 2 * 2
    expect(getEnrageBonus(10, 3, 1.0)).toBe(6);  // turn 10: 3 * 2
  });
  it('escalates to phase 2 after 3 enrage turns', () => {
    expect(getEnrageBonus(11, 3, 1.0)).toBe(10); // 3*2 + 1*4
    expect(getEnrageBonus(12, 3, 1.0)).toBe(14); // 3*2 + 2*4
  });

  // Deep (floors 7-12): enrage starts turn 6
  it('starts enrage earlier on deep floors', () => {
    expect(getEnrageBonus(5, 9, 1.0)).toBe(0);
    expect(getEnrageBonus(6, 9, 1.0)).toBe(2);
  });

  // Abyss (floors 13-18): enrage starts turn 5
  it('starts enrage at turn 5 on abyss floors', () => {
    expect(getEnrageBonus(4, 15, 1.0)).toBe(0);
    expect(getEnrageBonus(5, 15, 1.0)).toBe(2);
  });
  ...
  it('starts enrage at turn 4 on archive floors', () => {
    ...
    expect(getEnrageBonus(4, 20, 1.0)).toBe(2);
  });
  ...
  it('adds low HP bonus when enemy below 30%', () => {
    expect(getEnrageBonus(7, 3, 0.29)).toBe(3);   // no enrage (startTurn=8), just low HP bonus=3
    expect(getEnrageBonus(8, 3, 0.29)).toBe(5);   // 2 (enrage) + 3 (low HP)
  });
  it('does not add low HP bonus when enemy at or above 30%', () => {
    expect(getEnrageBonus(8, 3, 0.30)).toBe(2);   // just enrage, no low HP
    expect(getEnrageBonus(8, 3, 0.50)).toBe(2);
  });
});
