...
**Status:** COMPLETE — remaining verification/regeneration work moved to AR-121
...
- [x] **A2.** Create `data/curated/{domain}/entities-enriched.json` schema and storage — enriched entities stored per-domain during pilot
- [ ] ~~**A3.** Create `scripts/content-pipeline/verify-sources.mjs`~~ — moved to AR-121
...
- [x] **A5.** Update the ingest skill (`manual-fact-ingest-dedup/SKILL.md`) to reference new pipeline
...
- [x] **B4.** Compare quality against the old hallucinated facts for the same entities — pilot batches confirmed grounded quality; 4,681 facts marked sourceVerified
- [x] **B5.** Adjust the master system prompt based on pilot results
- [ ] ~~**B6.** Run source verification gate~~ — moved to AR-121

### Phase C: Scale Decision (after pilot)

- [ ] ~~**C1.** Full regeneration decision~~ — moved to AR-121 (knowledge facts only; vocab excluded)
- [x] **C2.** Flag existing facts as `sourceVerified: false` — `source_verified` column added to DB; 105,977 facts default to 0 (unverified), 4,681 marked verified
- [ ] ~~**C3.** Regenerate domain-by-domain~~ — moved to AR-121
- [ ] ~~**C4.** Rebuild DB after each domain~~ — moved to AR-121
