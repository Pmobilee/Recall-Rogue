...
**BUG16 — transport listener accumulation on leave + rejoin** (`src/services/multiplayerLobbyService.ts:1312`): `setupMessageHandlers` used a single `_handlersAttached` boolean to prevent double-registration in a single lobby lifetime. On `leaveLobby`, `_handlersAttached = false` was set — but the old listeners were still on the singleton transport. On the next `joinLobby`, each handler fired N times (N = number of join/leave cycles). Fix: every `transport.on()` in `setupMessageHandlers` now stores its cleanup in `_activeHandlerCleanups[]`. On `leaveLobby`, all cleanups are called before resetting `_handlersAttached`. A `reg()` helper encapsulates the pattern. Regression test added.

**BUG17 — LobbyChatUpdate only handles Entered, not Left** (`src-tauri/src/steam.rs:225`): The LobbyChatUpdate callback only fired for `Entered`. When a peer crashed or lost network (no `mp:lobby:leave` sent), the host's `_currentLobby.players` kept the ghost entry forever. Fix: added `pending_peer_left: Arc<Mutex<Option<u64>>>` to `SteamState`. The else-branch of the callback now writes the peer SteamID for Left/Disconnected/Kicked/Banned. New Tauri command `steam_get_pending_peer_left()` (take() semantics). New `getPendingPeerLeft()` TS helper in `steamNetworkingService.ts`. A 1s poll in `setupMessageHandlers` reads the slot and synthesises a local player removal.

**BUG18 — zero-byte primer messages logged as malformed** (`src/services/multiplayerTransport.ts:608-615`): `JSON.parse('')` throws. Zero-byte primers from `session_request_callback` and `steam_prime_p2p_sessions` are expected handshake messages, not bugs. Fix: early return with `rrLog` before the JSON.parse try/catch when `data.length === 0`.

**BUG19 — LAN state never published to debug overlay** (`src/services/lanServerService.ts`): `lanServerService.ts` had no calls to `setMpDebugState`. The MpDebugOverlay showed "no lan state" forever even when LAN was running. Fix: `startLanServer()` publishes `{ lan: { boundUrl, lastProbeResult, lastError } }` on success; `stopLanServer()` clears with `setMpDebugState({ lan: null })`. Added `import { setMpDebugState }` to `lanServerService.ts`.

**BUG20 — no console dump command** (`src/services/mpDebugState.ts`): No quick way to dump the full MP debug state for log-sharing. Fix: `window.__rrMpDebug()` global function added at module eval time. Calls `console.log('[rrMpDebug]', JSON.stringify(state, null, 2))` and returns the state object.
