...
### 2026-04-03 — Medical Terminology deck scoped from LLM memory, not a real curriculum
**What went wrong:** The Medical Terminology deck (635 facts) was built by compiling prefix/suffix/root lists from web search results and LLM knowledge. No actual medical terminology textbook or course curriculum was consulted. The deck completely missed medical abbreviations (STAT, PRN, BID, NPO, IV, IM, PO) — terms every nursing student uses daily.
**Why:** The orchestrator treated "research the full scope" as "search the web and compile a list" rather than "find an authoritative curriculum and extract its complete contents." LLMs don't know what they don't know — without a curriculum source, invisible gaps are guaranteed.
**Fix:** For any deck where students depend on completeness (medical, language, certifications), the scope MUST come from an authoritative curriculum source (textbook ToC, official word list, exam blueprint) — never from LLM compilation. Find the canonical source first, extract complete scope from it, cross-reference 2+ sources.
**Rule:** Added to content-pipeline.md, deck-master skill (Absolute Rule #2), and memory. This rule already existed implicitly for language decks (JLPT/HSK word lists) but was not applied to medical content.

### 2026-04-03 — Deck assembly silently dropped pools and shipped under architecture targets
...
### 2026-04-03 — overcharge_bonus_x2 tag is a turnManager signal, not resolver-implemented
