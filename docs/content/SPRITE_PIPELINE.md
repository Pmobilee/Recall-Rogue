...
`scripts/process-icons.mjs` processes raw 1024×1024 icon artwork from the sprite generator into game-ready assets at two resolutions.

### Input
Source files must be named `*_original.png` and placed in `sprite-gen/output/icons/`. These are generated by ComfyUI at 1024×1024.

### Processing steps
1. **Stray pixel removal** — alpha-threshold pass (pixels with alpha < 20 cleared), then 3×3 neighbourhood scan removes isolated pixels with fewer than 2 opaque neighbours.
2. **Transparent border trim** — tight crop to the bounding box of non-transparent pixels.
3. **Dual-resolution resize** — 128×128 for lo-res (`public/assets/sprites/icons/`) and 256×256 for hi-res (`public/assets/sprites-hires/icons/`).
4. **Format output** — PNG and WebP written for each resolution.

### Usage
```bash
# Process all *_original.png files in sprite-gen/output/icons/
node scripts/process-icons.mjs

# Show what would be processed without writing any files
node scripts/process-icons.mjs --dry-run

# Check which deployed icons have zero code references
node scripts/process-icons.mjs --audit
```

### Outputs
| Path | Size | Use |
|------|------|-----|
| `public/assets/sprites/icons/*.png` | 128×128 | In-game UI (default load path) |
| `public/assets/sprites/icons/*.webp` | 128×128 | WebP variant for browsers that prefer it |
| `public/assets/sprites-hires/icons/*.png` | 256×256 | Popup/tooltip enlargements, hi-DPI displays |
| `public/assets/sprites-hires/icons/*.webp` | 256×256 | WebP variant |

`InRunTopBar.svelte` loads hi-res paths directly for status icons (poison, weakness, vulnerable, strength, regen, immunity) where the extra resolution is visible at topbar scale.

### Poison brightness fix
The CSS `filter: brightness(1.5)` hack on poison icons was removed after all icons were reprocessed through this pipeline. The brightness was compensating for raw-generation darkness artifacts that the stray-pixel + trim + resize pipeline now eliminates at source.
