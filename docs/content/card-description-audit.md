...
4. **Description hides QP/CC/CW variance:** Many mechanic descriptions only show the QP scenario or use the phrasing "CC: higher values" without stating numbers. This is correct prose style for some mechanics but a genuine problem on others where the CC branch does something qualitatively different (exhaust, lockout, self-damage reversal).
...
| reckless | Reckless | attack | reads-stat-table | B, C, F | "Deal 4 damage. Take 3 self-damage." | Applies `qpValue=4`, selfDmg=4 from extras | B: selfDmg shown as 3 (seed secondaryValue), stat table L0 has selfDmg=4; C: CW shows same self-damage as QP |
| execute | Execute | attack | A (partial), D, F | — | "Deal 4 damage. +8 bonus if enemy below 30% HP." | Hardcodes `bonusBaseValue` as 24/8/4 for CC/QP/CW | A: resolver hardcodes bonus literal instead of stat-table extras.execBonus (L0 execBonus=4) |
...
| double_strike | Double Strike | buff | reads-stat-table | C, D, F | "Next attack card hits twice at full power (100% each)." | Applies applyDoubleStrikeBuff; CC: doubleStrikeAddsPierce=true | C: CC also pierces — not shown; D: double_strike_pierce (L5) not shown; F: seed quickPlayValue=75 but stat table L0 extras.hitMult=75 |
...
| inscription_fury | Inscription of Fury | buff | reads-stat-table | C, D, F | "Exhausts on play. All attacks deal +1 bonus damage for the rest of combat. CC: double bonus." | finalValue = qpValue=1 at L0; CC "double" via standard pipeline | D: insc_fury_cc_bonus2 (L5 CC gets +2 extra flat damage) not shown |
| inscription_iron | Inscription of Iron | buff | reads-stat-table | C, D, F | "Exhausts on play. Gain +1 Block at the start of each turn for the rest of combat. CC: double." | finalValue = qpValue=1 at L0 | D: insc_iron_thorns1 (L5) not shown |
...
| volatile_slash | Volatile Slash | attack | reads-stat-table | C, D, F | "Deal 4 damage. CC: 7 damage then Exhaust this card. L5: CC no longer Exhausts." | Applies `qpValue=4` at L0; CC: `finalValue` (= qpValue × 1.5 = 6) then exhaustOnResolve | C: CC damage shown as `power × 1.75` = baseEffectValue×1.75=17.5 — wrong; resolver uses standard pipeline CC = 6 at L0; D: description is already somewhat correct but CC value is imprecise |
| burnout_shield | Burnout Shield | shield | reads-stat-table | C, D, F | "Gain 4 Block. CC: 7 Block then Exhaust this card. L5: CC no longer Exhausts." | Applies `qpValue=5` at L0; CC = 5 × 1.5 = 7.5 | B/C: description shows `power=baseEffectValue=8`, so CC shows 14 — wrong; resolver L0 CC = 7.5 ≈ 7; D: burnout_no_exhaust mentioned in description |
...
| hemorrhage | Hemorrhage | attack | A, C, F | — | "Deal 4 damage plus 3× enemy Bleed stacks as bonus damage, then consume all Bleed. CC: higher multiplier." | hemoBase = mechanic.baseValue + getMasteryBaseBonus (deprecated) | A: uses deprecated getMasteryBaseBonus instead of stat-table qpValue; bleedMult hardcodes 6/2/4 not stat-table extras.bleedMult |
...
| bulwark | Bulwark | shield | reads-stat-table | C, D, F | "Gain 9 Block. CC: 15 Block then Exhaust this card. L3+: CC no longer Exhausts." | Applies qpValue from stat table; CC = qpValue × 1.5; L3+ bulwark_no_exhaust | C: CC value shown as `power × 1.75` = baseEffectValue×1.75=15.75 → 15 — almost correct by coincidence (stat table L0 CC = 9×1.5=13.5); B: description says "Gain 9 Block" and stat table L0 qpValue=9 — correct |
...
| recollect | Recollect | utility | reads-stat-table | C, D, F | "Return 1 exhausted card(s) to your discard pile. CC: return 2." | exhaustedCardsToReturn from stat-table (indirectly via mastery check) | D: recollect_upgrade1 (L3), recollect_play_free (L5) not shown |
