# Card Description Audit — 2026-04-11 (updated 2026-04-12: Wave 1-4 description fixes)
...
| multi_hit | Multi-Hit | attack | reads-stat-table | B, D, F | **Fixed 2026-04-12:** all 3 functions now read `stats?.hitCount ?? secondary ?? 3` for hit count | Applies `qpValue=2` per hit, `hitCount=2` | B: power still reads stat-table (qpValue); hit count now correctly reads hitCount from stat table |
...
| piercing | Piercing | attack | reads-stat-table | D, F | **Fixed 2026-04-12:** detailed desc reads `pierce_strip3` (L3) and `pierce_vuln1` (L5) tags | Applies `qpValue=3` pierce | D: FIXED — both L3 strip and L5 vuln now shown in detailed desc |
...
| block | Block | shield | reads-stat-table | B, D, F | **Fixed 2026-04-12:** `getDetailedCardDescription` now shows `block_consecutive3` bonus at L5 | Applies `qpValue=4` | D: FIXED — consecutive bonus now shown in detailed desc; B: reads from stat table via power var |
| thorns | Thorns | shield | A (partial), B, C, F | — | **Fixed 2026-04-12:** all 3 desc functions now read `stats?.secondaryValue ?? secondary ?? 3` | Uses seed secondaryValue=3 for reflect not stat table | A/B: resolver still hardcodes CC/CW reflect values; description now reads stat table secondaryValue (L0=3, which matches fallback) |
...
| parry | Parry | shield | reads-stat-table | B, C, D, F | **Fixed 2026-04-12:** draw count now reads `stats?.secondaryValue ?? 1` in all 3 functions (L3+: draws 2) | Applies `qpValue=1` | B: reads qpValue correctly now; D: parry_counter3 (L5) still not shown |
| empower | Empower | buff | reads-stat-table | B, C, F | **Fixed 2026-04-12:** detailed desc now reads `empower_2cards` tag (L3+: boosts 2 cards) | Applies `qpValue=30` (stat table) | B: reads stat table correctly; C: CC vs QP% still hidden; D: now shows 2-card boost at L3+ |
| quicken | Quicken | buff | reads-stat-table | C, D, F | **Fixed 2026-04-12:** detailed desc reads draw tags and ap2 tag; shows draw count and +2 AP at L5 | Applies grantsAp=1 (or 2 at L5) | D: FIXED — quicken_draw1/draw2/ap2 tags now reflected in detailed desc; C: CC/QP/CW still same behavior |
| focus | Focus | buff | reads-stat-table | C, D, F | **Fixed 2026-04-12:** detailed desc reads focus_ap0 (free at L3) and focus_next2free (L5: next 2 cards free) tags | CC: focusCharges=2 (two cards get AP reduction) | D: PARTIALLY FIXED — L3 free and L5 2-cards shown; C: CC 2-card focus still not shown |
...
| weaken | Weaken | debuff | reads-stat-table | B, C, D, F | **Fixed 2026-04-12:** all 3 desc functions now read `extras.turns` for duration | Applies stacks + duration from extras | B: FIXED — turn count now shown; D: weaken_shield30 (L5) still not shown in description |
| expose | Expose | debuff | reads-stat-table | B, C, D, F | **Fixed 2026-04-12:** all 3 desc functions now read `extras.turns` for duration | Applies stacks + duration from extras | B: FIXED — turn count now shown; D: expose_dmg3, expose_vuln75 still not shown |
| hex | Hex | debuff | A, B, F | — | **Fixed 2026-04-12:** all 3 desc functions now read `stats?.extras?.['turns'] ?? secondary ?? 3` | Resolver hardcodes poisonValue as 8/2/3 for CC/CW/QP | A: hardcodes remain in resolver; description turns now match stat table |
...
| scout | Scout | utility | reads-stat-table | B, D, F | **Fixed 2026-04-12:** all 3 desc functions now read `stats?.drawCount` for draw count | extraCardsDrawn from stat-table drawCount=1 (L0) | B: FIXED — description now reads stat table drawCount directly; D: scout_scry2 (L3/L5) still not shown |
...
| transmute | Transmute | utility | reads-stat-table | D, F | **Fixed 2026-04-12:** detailed desc reads `extras.transforms` to show L5 2-card transform | pendingCardPick or applyTransmuteAuto | D: PARTIALLY FIXED — L5 2-card transform now shown; L2 choose and L3 +1 mastery still not shown |
| immunity | Immunity | utility | reads-stat-table | B, D, F | **Fixed 2026-04-12:** all 3 desc functions now read `stats?.extras?.['absorb'] ?? 4` | applyImmunity=true (limit from stat table extras.absorb) | B: FIXED — description now reads absorb from stat table; D: immunity_reflect50 (L5) still not shown |
...
| iron_wave | Iron Wave | attack | A (partial), B, C, F | — | **Fixed 2026-04-12:** fallback for secondaryValue corrected ?? 3 → ?? 5 to match L0 stat table | Applies `qpValue=2` dmg; block uses deprecated getMasterySecondaryBonus for CC | A: CC block still uses deprecated helper; fallback now correct |
...
| bash | Bash | attack | reads-stat-table | B, C, D, F | **Fixed 2026-04-12:** detailed desc and parts now read `bash_vuln2t` tag to show correct vuln duration | Applies `qpValue=4` + Vulnerable duration from tag | D: bash_weak1t (L5 Weakness) still not shown in description |
...
| lacerate | Lacerate | debuff | A (partial), B, C, F | — | **Fixed 2026-04-12:** detailed desc reads `lacerate_vuln1t` tag (L5: adds Exposed 1t) | Resolver hardcodes `lacerateBleed` as 8/2/`secondaryValue` | A: hardcode remains in resolver; D: FIXED — L5 vuln now mentioned in detailed desc |
| kindle | Kindle | attack | A, B, F | — | **Fixed 2026-04-12:** fallback for secondaryValue corrected ?? 2 → ?? 4 to match L0 stat table | Resolver hardcodes `kindleBurn` as 8/2/`secondaryValue` | A: Burn stacks still hardcoded in resolver; description fallback corrected |
...
| overcharge | Overcharge | attack | A, B, C, F | — | **Fixed 2026-04-12:** detailed desc reads `overcharge_bonus_x2` tag (L3+: bonus doubled) | Resolver ignores `finalValue` for CC; turnManager handles CC scaling | A: hardcode remains in resolver; D: FIXED — L3 doubled bonus now mentioned in detailed desc |
| riposte | Riposte | attack | A (partial), B, D, F | — | **Fixed 2026-04-12:** fallback for secondaryValue corrected ?? 3 → ?? 4 to match L0 stat table | CC block hardcoded as `12 × focusAdjustedMultiplier` | A: CC block still hardcoded in resolver; description fallback corrected |
...
| reactive_shield | Reactive Shield | shield | A (partial), B, C, F | — | **Fixed 2026-04-12:** fallback for secondaryValue corrected ?? 1 → ?? 2 to match L0 stat table | CC: rsThornValue=5 (hardcoded), CW=1 (hardcoded), QP from secondaryValue | A: CC/CW thorns still hardcoded in resolver; description fallback corrected |
...
| scavenge | Scavenge | utility | reads-stat-table | D, F | **Fixed 2026-04-12:** detailed desc reads `extras.picks` (L3: 2 cards) | pendingCardPick from discard | D: PARTIALLY FIXED — pick count now shows L3 multi-pick; L2 draw1 and L5 AP free still not shown |
...
| warcry | Warcry | buff | A, C, D, F | — | **Fixed 2026-04-12:** detailed desc reads `warcry_freecharge` tag (L3+: free charge on CC) and `str` from stat table | CC: hardcodes `{ value: 2, permanent: true }` + warcryFreeCharge; QP: hardcodes `{ value: 2, permanent: false }` | A: resolver still hardcodes str values; D: PARTIALLY FIXED — freecharge now shown at L3; perm_str at L3 QP still not shown |
| battle_trance | Battle Trance | buff | reads-stat-table | C, D, F | **Fixed 2026-04-12:** detailed desc reads `trance_no_lockout_qp` (L3) and `trance_cc_ap1` (L5) tags | drawCount from stat table (L0=2); lockout via applyBattleTranceRestriction | D: FIXED — L3 QP no lockout and L5 CC+1AP now shown |
...
| chameleon | Chameleon | wild | reads-stat-table | D, F | **Fixed 2026-04-12:** detailed desc reads `extras.qpMult/ccMult/cwMult`; L0 shows 70%/100%/50% | chameleonMultiplier from hardcoded literals, stat-table extras used by turnManager | C: FIXED — description now reads stat-table values; D: chameleon_chain (L5) still not shown |
