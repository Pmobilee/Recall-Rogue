...
## Post-Audit Fix Results (Same Session)

### Fixes Applied

| Fix ID | Issue | Status | Impact |
|--------|-------|--------|--------|
| FIX-01 | Surge AP surcharge never applied | FIXED | Charging now costs +1 AP on non-surge turns; surge waives it |
| FIX-02 | Balance sim validation | DONE | Win rates dropped ~40% (developing: 53%→13%, experienced: 85%→40%) |
| FIX-03 | Shop frequency too low | FIXED | SHOP_MIN_SPACING 2→1, SHOP_MAX_COUNT 2→3; shops now 0.5-0.6/act |
| FIX-04 | Relic V1→V2 migration not called | FIXED | Migration now runs on load() with version check |
| FIX-05 | HubScreen non-reactive let | FIXED | hubCenterEl/campHubEl now $state() |
| FIX-06 | ShopRoomOverlay state_referenced_locally | FIXED | Used untrack() pattern |
| FIX-07 | CardHand state_referenced_locally | FIXED | Used untrack() pattern |
| FIX-08 | CustomDeckViewModal state_referenced_locally | FIXED | Used untrack() pattern |
| FIX-09 | No treasure room guarantee | FIXED | Added TREASURE_MIN_COUNT=1 with enforcement |
| FIX-10 | 8 unused CSS selectors | FIXED | Removed from 6 components |
| FIX-11 | MultiChoicePopup missing tabindex | FIXED | Added tabindex="-1" |

### New Tests Written

| Test File | Tests Added | Coverage |
|-----------|-------------|----------|
| tier-derivation.test.ts | 33 | FSRS tier transitions, mastery trial |
| confusion-matrix.test.ts | 24 | Serialization round-trip, bidirectional lookup |
| surcharge-regression.test.ts | 56 | Surcharge applied/waived per turn, isSurgeTurn consistency |
| saveService.migration.test.ts | 4 | V1→V2 migration wiring, idempotency |

### Post-Fix Metrics

| Metric | Before Fixes | After Fixes | Change |
|--------|-------------|-------------|--------|
| Tests passing | 3567/3567 | 3689/3689 | +122 tests |
| Type errors | 0 | 0 | — |
| Type warnings | 30 | 15 | -15 (fixed) |
| Build time | 9.48s | 8.66s | -0.82s |
| Deck failures | 0 | 0 | — |

### Post-Fix Balance (6000 runs with surcharge + shop fixes)

| Profile | Pre-Fix WR | Post-Fix WR | Change |
|---------|-----------|-------------|--------|
| new_player | 0% | 0% | — |
| developing | 53% | 13% | -40% |
| competent | 51% | 15% | -36% |
| experienced | 85% | 38% | -47% |
| master | 100% | 94% | -6% |
| language_learner | 1% | 0% | -1% |

**Note:** Win rates dropped significantly because CHARGE_AP_SURCHARGE=1 was never previously applied. The game was balanced around surcharge=0 for months. The surcharge value may need tuning (e.g., to 0.5 or 0) to restore target win rates. The fix itself is correct — the question is what surcharge value the game should have.

### Anomalies Resolved

- **Competent vs Developing**: With surcharge fix, competent (15%) now correctly beats developing (13%). The previous anomaly (51% vs 53%) was an artifact of the surcharge bug making the game too easy, where small behavioral differences didn't matter.
- **Language learner 0% WR**: Confirmed intentional. 35% accuracy means you can't answer most questions — this correctly models a player learning a completely new language from scratch.

### Remaining Open Items

| Item | Priority | Status |
|------|----------|--------|
| 87 card mechanics lack dedicated unit tests | HIGH | 6 test agents running in parallel (worktrees) |
| 65 missing sprite references | MEDIUM | Not addressed (art generation needed) |
| Combat SFX sparse (1 file) | MEDIUM | Not addressed (audio curation needed) |
| 346 inspection registry items untested | MEDIUM | Not addressed (requires /inspect cycles) |
| Surcharge value may need retuning | HIGH | User decision needed — 1 may be too harsh |
| Combat chunk 3.1MB | LOW | Not addressed (code-splitting) |

---

*Generated by Claude Opus 4.6 autonomous audit — 2026-04-08*
*Post-audit fixes applied in same session*
