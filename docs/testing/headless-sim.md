...
Reports 1-6 are the original survivorship-biased reports. Reports 7-9 are survivorship-free multi-dimensional reports added 2026-04-09.
...
| 8 | `card-performance.md` | Markdown | **Survivorship-free per-card efficiency.** avgDmgPerAP (AP-normalised), avgBlockPerPlay, chargeRate, chargeAccuracy, playFrequency, floorDelta (avgFloorsWithCard - avgFloorsWithout). Split into attack / shield / utility sections. Requires 50+ plays. |
...
> **Data model status (2026-04-09):** All fields are now populated. `RelicAcquisition` tracking, `relicsHeld`/`playerHpBefore`/`apAvailable`/`apSpent`/`chargeRate`/`chargeAccuracy` on `EncounterDetail`, and `blockGained`/`apCost`/`effectValue`/`chainLength` on `CardPlayRecord` are all fully wired in `full-run-simulator.ts` and `simulator.ts`. Reports 7-9 consume these fields without approximation.
## Sweep Mode
