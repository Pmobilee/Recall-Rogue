<!-- 
  Purpose: Master visual + functional verification checklist for Docker-based testing.
...
# Recall Rogue — Visual + Functional Verification Checklist

This is the master TODO checklist for every visual and mechanical element testable via Docker visual tests. 885 items across 18 sections, plus the priority chain multiplier rework spec below.

---

# IMPLEMENT FIRST: Chain Multiplier Rework

> This rework must land before the rest of the checklist is meaningful, because it changes the damage/block/DoT formulas that every card item below tests against.
