...
### Enemy Roster Summary (v2)

Enemies are organized into 3 acts with act-based pools replacing the old region system. The v2 roster introduces **quiz-reactive callbacks** — enemies that respond to how the player plays cards (Charge vs Quick Play, correct vs wrong answers).

#### Act 1 — The Depths (floors 1-4)
| ID | Category | BaseHP | Key Behavior |
|----|----------|--------|--------------|
| cave_bat | common | 19 | Standard swooper |
| cave_bat_alpha | common | 28 | Stronger variant |
| dusk_bat | common | 22 | Variant of cave_bat |
| crystal_golem | common | 38 | High defense, slammer |
| iron_golem | common | 52 | Stronger variant |
| toxic_spore | common | 15 | Poison debuffer |
| poison_bloom | common | 20 | Variant of toxic_spore |
| timer_wyrm | mini_boss / boss | 45 | **Enrage**: +5 damage/turn from turn 4 onward |

#### Act 2 — The Archive Approach (floors 5-8)
| ID | Category | BaseHP | Key Behavior |
|----|----------|--------|--------------|
| shadow_mimic | common | 28 | **Mirror**: wrong Charge → deals card's base damage back to player |
| dark_shade | common | 22 | Variant of shadow_mimic |
| bone_collector | common | 35 | **Lifesteal**: wrong Charge → heals 5 HP (no overheal) |
| grave_warden | common | 42 | Variant of bone_collector |
| the_examiner | elite | 55 | **Punish**: no Charge this turn → stacks +3 Strength (encounter-permanent) |
| the_archivist | boss | 80 | Multi-phase boss; quiz gauntlet at 50% HP (5 questions) |

#### Act 3 — The Archive (floors 9+)
| ID | Category | BaseHP | Key Behavior |
|----|----------|--------|--------------|
| the_scholar | common | 40 | **Reward**: correct Charge → heals 5 HP |
| lore_keeper | common | 55 | Variant of the_scholar |
| the_nullifier | elite | 70 | **Chain Nullifier**: chain multiplier capped at 1.0x |
| the_librarian | elite | 65 | **Quick Immune**: Quick Play deals 0 damage (must Charge) |
| the_curator | boss | 120 | Multi-phase boss; quiz gauntlet at 66% and 33% HP (rapid fire at 33%) |

#### Quiz-Reactive Enemy Callbacks

Enemies can define any of the following callbacks on their template:

| Callback | Fires When |
|----------|------------|
| `onPlayerChargeWrong` | Player played a card as Charge and answered wrong |
| `onPlayerChargeCorrect` | Player played a card as Charge and answered correctly |
| `onPlayerNoCharge` | Player ended their turn without using any Charge play |
| `onEnemyTurnStart` | Start of the enemy's turn (used for enrage/per-turn effects) |

All callbacks receive an `EnemyReactContext` (enemy, cardBaseDamage, playMode, chargeCorrect) or `EnemyTurnStartContext` (enemy, turnNumber). The Shadow Mimic uses a side-channel `_mirrorDamage` property on the context, read by the turn manager after the callback returns.

#### Passive Enemy Flags

| Flag | Effect |
|------|--------|
| `quickPlayImmune: true` | Quick Play deals 0 damage to this enemy (e.g. The Librarian) |
| `chainMultiplierOverride: number` | Overrides the chain combo multiplier (e.g. The Nullifier sets 1.0x) |

#### Act Enemy Pools (`ACT_ENEMY_POOLS`)

Enemy selection for map nodes uses the new `getEnemiesForNode(act, nodeType)` function which reads from `ACT_ENEMY_POOLS`. The floor-to-act mapping is in `floorManager.ts` (`getActForFloor`): floors 1-4 → Act 1, floors 5-8 → Act 2, floors 9+ → Act 3.
...
### Relic Catalogue (V2 — AR-59.11)

**42 relics total**: 25 free (available to all players) + 17 unlockable (purchased with Mastery Coins). Organized around 7 build archetypes.

**Categories:** Offensive, Defensive, Sustain, Tactical, Knowledge, Economy, Cursed, Chain, Speed, Burst, Poison, Glass Cannon.
**Rarities:** Common (11), Uncommon (14 free + 3 paid), Rare (15 paid), Legendary (2 paid).
**Data:** `src/data/relics/starters.ts` (25 free), `src/data/relics/unlockable.ts` (17 unlockable + toxic_bloom Phase 2).

#### Build Archetypes

| # | Archetype | Fantasy | Core Relics |
|---|-----------|---------|-------------|
| 1 | **Chain Master** | Long chains, exponential damage | chain_reactor, resonance_crystal, prismatic_shard |
| 2 | **Speed Scholar** | Fast answers, massive bonuses | quicksilver_quill, adrenaline_shard, time_warp |
| 3 | **Glass Cannon** | High risk, one-shot kills | volatile_core, reckless_resolve, crit_lens |
| 4 | **Iron Fortress** | Unkillable, attrition wins | aegis_stone, thorn_crown, regeneration_orb |
| 5 | **Poison Alchemist** | DoT stacking, indirect damage | plague_flask, festering_wound |
| 6 | **Burst Master** | Save AP, nuke on one massive turn | capacitor, overflow_gem, double_down |
| 7 | **Knowledge Engine** | Quiz mastery = combat mastery | scholars_crown, memory_nexus, insight_prism |

#### Free Starters (25, cost 0) — Commons (11) + Uncommons (14)

| ID | Name | Rarity | Category | Effect |
|----|------|--------|----------|--------|
| whetstone | Whetstone | Common | Offensive | All attack cards +2 base damage |
| iron_shield | Iron Shield | Common | Defensive | Start each turn with 3 block |
| vitality_ring | Vitality Ring | Common | Sustain | +12 max HP this run |
| herbal_pouch | Herbal Pouch | Common | Sustain | Heal 4 HP after each combat encounter |
| swift_boots | Swift Boots | Common | Tactical | Draw 6 cards per turn instead of 5 |
| combo_ring | Combo Ring | Common | Offensive | First Charged correct/turn: +2 damage to all attacks that turn |
| steel_skin | Steel Skin | Common | Defensive | Take 1 less damage from all sources (min 1) |
| gold_magnet | Gold Magnet | Common | Economy | +30% gold from all sources |
| merchants_favor | Merchant's Favor | Common | Economy | Shops offer +1 card choice and +1 relic choice |
| lucky_coin | Lucky Coin | Common | Economy | Start each encounter with 1 random buff (Empower/+2 block/+1 AP/Draw 1) |
| scavengers_eye | Scavenger's Eye | Common | Economy | 4 card choices after combat instead of 3 |
| last_breath | Last Breath | Uncommon | Sustain | Once/encounter: survive lethal at 1 HP, gain 8 block |
| tag_magnet | Tag Magnet | Uncommon | Chain | +30% draw bias toward same-category cards |
| adrenaline_shard | Adrenaline Shard | Uncommon | Speed | Fast Charged correct (<3s): refund 1 AP, once/turn |
| volatile_core | Volatile Core (Cursed) | Uncommon | Glass Cannon | All attacks +40%; wrong Charge deals 5 to self AND enemy |
| aegis_stone | Aegis Stone | Uncommon | Defensive | Block carries (max 25); at 25 block gain Thorns 3 |
| regeneration_orb | Regeneration Orb | Uncommon | Defensive | Heal 3 HP at turn end if 2+ shield cards played |
| plague_flask | Plague Flask | Uncommon | Poison | Poison ticks +2 damage; poison lasts 1 extra turn |
| memory_nexus | Memory Nexus | Uncommon | Knowledge | 3rd Charged correct in encounter: draw 2 extra next turn |
| insight_prism | Insight Prism | Uncommon | Knowledge | Wrong Charge reveals answer; next same-fact auto-succeeds |
| blood_price | Blood Price (Cursed) | Uncommon | Cursed | +1 AP per turn; lose 2 HP per turn |
| reckless_resolve | Reckless Resolve | Uncommon | Glass Cannon | <40% HP: attacks +50%; >80% HP: attacks -15% |
| overflow_gem | Overflow Gem | Uncommon | Burst | Spend 4+ AP in one turn: last card +75% effect |
| resonance_crystal | Resonance Crystal | Uncommon | Chain | Each chain link beyond 2 draws +1 card at turn end |

#### Mastery Coin Unlocks (17, costs 55–75)

| ID | Name | Rarity | Category | Effect | Cost |
|----|------|--------|----------|--------|------|
| chain_reactor | Chain Reactor | Rare | Chain | Chains 3+: 4 splash dmg per link | 55 |
| echo_chamber | Echo Chamber | Rare | Chain | 3+ chain: replay first card at 50% power | 55 |
| quicksilver_quill | Quicksilver Quill | Rare | Speed | Charged correct <2s: ×1.5 extra multiplier | 55 |
| time_warp | Time Warp | Rare | Speed | Surge turns: timer halved, Charge multiplier 4.0× | 55 |
| crit_lens | Crit Lens | Rare | Glass Cannon | Charged correct: 25% chance to double final damage | 55 |
| thorn_crown | Thorn Crown | Rare | Defensive | 15+ block: reflect 5 damage per enemy attack | 55 |
| bastions_will | Bastion's Will | Rare | Defensive | Charged shield cards: +50% block value | 55 |
| festering_wound | Festering Wound | Rare | Poison | 5+ enemy poison: all attacks +30% damage | 55 |
| capacitor | Capacitor | Rare | Burst | Store unused AP (max 3); release as bonus AP next turn | 55 |
| double_down | Double Down | Rare | Burst | Once/encounter: double-charge — both correct 5×, one 1.5×, both wrong 0.3× | 60 |
| scholars_crown | Scholar's Crown | Rare | Knowledge | Tier 2+ Charged: +30%; Tier 3 auto-Charged: +50% | 55 |
| domain_mastery_sigil | Domain Mastery Sigil | Rare | Knowledge | 6+ same-domain facts: +20% base damage to all same-domain cards | 55 |
| phoenix_feather | Phoenix Feather | Rare | Sustain | Once/run: resurrect at 30% HP; all cards auto-Charge free for 2 turns | 60 |
| scholars_gambit | Scholar's Gambit (Cursed) | Rare | Cursed | +1 relic slot (max 6); wrong Charged answers deal 3 self-damage | 60 |
| prismatic_shard | Prismatic Shard | Legendary | Chain | All chain multipliers +0.5×; 5-chains grant +1 AP | 75 |
| mirror_of_knowledge | Mirror of Knowledge | Legendary | Knowledge | Once/encounter: after correct Charge, replay card at 1.5× free | 75 |

> **Phase 2 only**: `toxic_bloom` (spread poison on kill) — defined with `excludeFromPhase1: true`, awaiting multi-enemy system.

#### Per-Run State Flags (V2 Relics)

| Flag | Type | Used by | Resets |
|------|------|---------|--------|
...
### Five-Slot Relic System (AR-59.10)

Runs begin with **0 relics**. The starter relic selection screen is removed — relics are acquired exclusively through encounters.

**Slot cap**: Players can equip a maximum of **5 relics** simultaneously. Scholar's Gambit (a cursed rare) expands this to **6**.

**At full capacity**: When a new relic is offered and the player is at 5/5 (or 6/6), the `RelicSwapOverlay` is triggered:
- Shows the offered relic and all 5 equipped relics with their sell prices.
- Tap any equipped relic to see an inline sell confirmation: "Sell for Xg?"
- **Confirm Sell** removes the selected relic, adds the refund to currency, and acquires the new relic.
- **Pass** returns to the dungeon map without acquiring the new relic.

**Sell refunds (40% of rarity shop price):**

| Rarity | Shop Price | Sell Refund |
|--------|-----------|-------------|
| Common | 60g | 24g |
| Uncommon | 100g | 40g |
| Rare | 160g | 64g |
...
**Relic reroll**: At boss, mini-boss (first), and elite relic selections, players can spend **50g** to reroll all 3 options once per selection event. The reroll button is disabled if gold < 50 or already used this selection. Rerolled options are added to a session-scoped `rerollSeenIds` set (cleared when the selection event ends) — they cannot reappear within the same reroll session. **Declined relics CAN reappear in later events** (scope change from AR-59.12 — only acquired relics are permanently excluded).

**Unlock tiers**: All 25 starter relics in `starters.ts` have `startsUnlocked: true` and appear in the run pool for all players. The ~27 relics in `unlockable.ts` require Mastery Coin purchases from the Hub Relic Archive.

**Excluded relics**: Relics with `excludeFromPool: true` are excluded from in-run drop pools (Phase 1 only). Currently `venom_fang` (requires multi-enemy poison tracking). They remain visible in the Hub Relic Archive but cannot appear as rewards.

**Scholar's Gambit** (new cursed rare relic):
- Grants +1 relic slot (max 6).
- Curse: wrong answers on Charged cards deal 3 self-damage (bypasses block).

### In-Run Acquisition (AR-59.12)

Runs start with **0 relics**. The starter relic selection screen is removed. The first relic opportunity is the Act 1 mini-boss (approximately encounter 8–10 depending on path taken). Hard cap of 5 slots (6 with Scholar's Gambit).
...
**Regular rarity weights:** Common 50%, Uncommon 30%, Rare 15%, Legendary 5%.
**Boss rarity weights:** Common 20%, Uncommon 35%, Rare 30%, Legendary 15%.

**Pity timer**: After 4 consecutive Common-only acquisitions (from any path), the next drop is guaranteed Uncommon or higher (`relicPityCounter` on `RunState`, threshold `RELIC_PITY_THRESHOLD = 4`). The counter resets to 0 on any Uncommon+ acquisition. Pity applies to both toast drops and choose-1-of-3 events (at least one option is forced Uncommon+).

**Pool exclusions**:
- Relics already held by the player are excluded
- Relics with `excludeFromPool: true` are excluded (used for relics requiring unbuilt mechanics)
- **Declined relics CAN reappear** — only acquired relics are permanently excluded from the pool. `offeredRelicIds` on `RunState` now only tracks acquired relics (AR-59.12 scope change from "all offered" to "acquired only").

**Full-capacity toast drops**: When a random drop (toast) occurs and the player is at 5/5 slots, the `RelicPickupToast` shows a "Swap" button. Tapping it opens `RelicSwapOverlay`. Dismissing the toast without swapping forfeits the relic (low-stakes, intentional).

Only `startsUnlocked: true` or explicitly unlocked relics are eligible. `excludeFromPool: true` relics are always excluded.
...
**Visual:** Translucent/ghostly appearance, dashed purple border, `echo-shimmer` animation. Badge shows "ECHO ⚡ CHARGE" to signal charge-only requirement. Clearly distinct from normal cards.

**Play restriction:** Echo cards can ONLY be played via Charge (quiz). Tapping a popped Echo card to Quick Play is blocked — a "Must Charge!" tooltip appears and the card remains selected.

**Spawn trigger:** Wrong Charge answers only. Quick Play has no quiz, so no Echoes are ever created from Quick Play.

**Echo card power at spawn:** Full base power (1.0×) — multiplier is applied at resolve time based on quiz outcome.

| Event | Power | FSRS | Other |
|-------|-------|------|-------|
| Correct Echo Charge | 1.0× full power | Double credit (6.0× bonus, `FSRS_STABILITY_BONUS_CORRECT_V2`) | Golden flash; fact removed from `echoFactIds` (cannot spawn again this run) |
| Wrong Echo Charge | 0.5× (`POWER_MULTIPLIER_WRONG`) | Standard miss (`'again'` button) | Card is **exhausted** — cannot be re-drawn this run |
...
| Power at spawn | Pre-baked 0.70× | Full original value; multiplier at resolve time |
| Power on correct Charge | 0.70× | 1.0× (full) |
| Power on wrong Charge | 0.70× (card still plays) | 0.5× + card exhausted |
| FSRS on correct | 3.0× bonus | 6.0× double credit |
| Echo spawn trigger | Any wrong answer | Wrong Charge only |

**`echo_lens` relic (V2):** Correct Echo Charge is already full power in V2 — `echo_lens` now prevents the 0.5× wrong-Charge penalty. Echo cards deal 1.0× regardless of quiz result.

**Insight Prism synergy:** When `insight_prism` relic is held AND a fact is in `insightPrismAutosucceedIds` (populated by a prior wrong Charge on the source card), the Echo quiz auto-succeeds: correct answer pre-highlighted for 300ms, then resolves as correct. One-time use per fact per run.

**Design effect:** Poor performance = more Echoes in your hand. But each Echo is now a meaningful high-stakes retry — answer correctly for full power redemption, or fail and lose the card permanently.
...
## 11a. Fact Discovery System — Free First Charge (AR-59.23)

The **Free First Charge** system solves the Charge cold-start problem: paying +1 AP to Charge a card you've never seen before is a blind guess. The solution: every fact's first Charge in a run is FREE.

### How It Works

- **Free Charge:** The first time a player Charges any fact in a run, the AP surcharge = 0 and wrong answers deal 1.0× (same as Quick Play — no penalty for trying).
- **Correct on free Charge:** Full Charge bonus applies normally (2.5×/3.0×/3.5× per tier).
- **After first Charge:** Normal Charge costs +1 AP surcharge; wrong answers deal the standard fizzle ratio (25%).
- **Tier 3 auto-Charge:** Does NOT consume the free Charge — the player didn't consciously choose to Charge.

### The ONLY Visual Indicator

The **CHARGE button** displays **"FREE"** (instead of "+1 AP") when a fact hasn't been Charged yet this run. No card glows, no hint text, no per-card visual states.

### Natural Run Arc

- **Act 1:** Most facts are new → CHARGE buttons show "FREE" → players explore what they know.
- **Act 2:** Mix of "FREE" and "+1 AP" → players selectively Charge facts they learned in Act 1.
- **Act 3:** Mostly "+1 AP" → players are veterans of their fact pool, Charge with confidence.

### Per-Run State

`firstChargeFreeFactIds: Set<string>` lives on `RunState`. Empty at run start. Once a factId is added, that fact costs normal +1 AP surcharge. Serialized to array on save, restored to Set on load.

### Balance Constants

| Constant | Value | Meaning |
|----------|-------|---------|
| `FIRST_CHARGE_FREE_AP_SURCHARGE` | 0 | AP surcharge for the first Charge of a new fact |
| `FIRST_CHARGE_FREE_WRONG_MULTIPLIER` | 1.0 | Wrong answer on free Charge = same as Quick Play |

### Files

| File | Role |
|------|------|
| `src/services/discoverySystem.ts` | `isFirstChargeFree()`, `markFirstChargeUsed()`, `getFirstChargeWrongMultiplier()` |
| `src/services/discoverySystem.test.ts` | 13 unit tests |
| `src/services/runManager.ts` | `firstChargeFreeFactIds: Set<string>` on RunState |
| `src/services/runSaveService.ts` | Serializes/deserializes firstChargeFreeFactIds as array |
| `src/services/turnManager.ts` | Checks free Charge before AP deduction; applies 1.0× on wrong; calls markFirstChargeUsed after resolution |
| `src/ui/components/CardHand.svelte` | CHARGE button shows "FREE" / "+1 AP" based on free Charge state |
| `src/data/balance.ts` | `FIRST_CHARGE_FREE_AP_SURCHARGE`, `FIRST_CHARGE_FREE_WRONG_MULTIPLIER` |
...
| Echo mechanic (V2) | Wrong Charge facts reappear as Charge-only ghost cards; redeemed at 1.0× or exhausted at 0.5× |
...
2. Run starts directly at the dungeon map with 0 relics — AR-59.12 removed the starter relic selection screen. First relic earned at Act 1 mini-boss.
...
## 21.6. Vocabulary Card System

### Card Structure
Each vocabulary card represents a word in one of 8 supported languages. Cards are created programmatically from open-source dictionary data — not LLM-generated.

### Required Fields Per Word
- `targetWord` — The word in the target language
- `englishTranslation` — English meaning (hub language)
- `reading` — Pronunciation guide (CJK only: pinyin, furigana, romanization)
- `partOfSpeech` — noun, verb, adjective, adverb, etc.
- `level` — Proficiency level (HSK / JLPT / TOPIK / CEFR)
- `frequencyRank` — Word frequency ranking (nice to have)

### Quiz Format
Players see one of four question types per vocab card:
1. **L2→L1 Meaning** (Tier 1, 3 options): "What does '猫' mean?" → [cat, dog, bird]
2. **L1→L2 Reverse** (Tier 2a, 4 options): "How do you say 'cat' in Chinese?" → [猫, 狗, 鸟, 鱼]
3. **Reading/CJK** (Tier 1, 3 options): "What is the reading of '猫'?" → [māo, gǒu, niǎo]
4. **Fill-blank** (Tier 2b, 5 options): "'___' means 'cat' in Chinese" → [猫, 狗, 鸟, 鱼, 马]

### Distractor Selection (Runtime)
Wrong answers are selected at runtime from the same-language vocabulary pool — NOT pre-generated by LLM:
- **POS match**: Same part of speech (nouns with nouns)
- **Level proximity**: Within ±1 level of the correct word
- **Semantic bins**: ~50 broad categories (animals, colors, food...) + ~200 narrow sub-categories
  - Easy difficulty → distractors from different bins
  - Medium difficulty → same broad bin, different sub-bin
  - Hard difficulty → same sub-bin (very tricky!)

### Difficulty Mapping
| HSK | JLPT | CEFR | Difficulty |
|-----|------|------|-----------|
| 1-2 | N5 | A1-A2 | 1 (Easy) |
| 3 | N4 | B1 | 2 (Medium) |
| 4 | N3 | B2 | 3 (Hard) |
...
ALL distractors MUST be generated by an LLM (GPT-5.2+ or Haiku agent) that reads the specific question, understands what's being asked, and produces plausible wrong answers that:
...
| Echo | Ghost card from wrong Charge answer. 85% reappearance chance. CHARGE ONLY. Full power on correct (1.0×), 0.5× and exhausted on wrong. |
