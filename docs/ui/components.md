...
| `CardVisual.svelte` | **Shared card frame renderer (2026-04-09).** Pure display-only component — no interaction, no drag, no state machine. Single source of truth for the V2 layered card frame used by both `CardHand.svelte` and `CardPickerOverlay.svelte`. **Props:** `card: Card`, `effectValue?: number`, `isChargePreview?: boolean`, `isBtnChargePreview?: boolean`, `modState?: 'buffed' | 'nerfed' | 'neutral' | null`, `masteryFlash?: 'up' | 'down' | null`, `displayedApCost?: string | number`, `apGemColor?: string | null`, `showMasteryGlow?: boolean`, `chainPillActive?: boolean`. **Renders:** 4-layer image stack (border + card art + base frame + chain banner), `.v2-mechanic-name` (with `use:stretchText`), `.v2-card-type` chain pill, `.v2-effect-text` parchment block (description parts rendered via `getCardDescriptionParts()` + `groupIntoLines()`), `.v2-ap-cost` gem. **CSS ownership:** ALL v2 card frame CSS lives here — callers must not duplicate it. `CardHand.svelte` uses `:global(.v2-ap-cost)` and `:global(.card-v2-frame)` only for cross-boundary interaction states (insufficient AP coloring, pointer-events on selected cards). **`--card-w` CSS variable** must be set on the wrapper by the caller. **Effect text font multipliers (2026-04-11):** Bumped ~22% at all four breakpoints — base: 0.095→0.115, `.effect-text-md`: 0.076→0.093, `.effect-text-sm`: 0.062→0.076, `.effect-text-xs`: 0.052→0.062 — improves legibility on ~170px overlay cards without overflowing combat-size (~290–340px) parchment. **mechanicName guide (2026-04-11):** `GUIDE_STYLES.mechanicName` in `cardFrameV2.ts` changed from `guideStyle(205, 120, 478, 110)` to `guideStyle(205, 80, 478, 110)` (y moved up 40 PSD units) so the name text centers on the banner ribbon. **mechanicName guide y update (2026-04-14):** Guide style y-coordinate further adjusted — the rendered CSS `top` position changed from 112px to 125px on the 1142px canvas (13px lower), centering the name text better on the ribbon rather than extending below it into the art window. `CardPickerOverlay` sets it via inline style matching `CardHand.landscapeCardW` formula: `(35vh * 0.88) / 1.42`; `CardHand` sets it per card slot based on the same formula. **Draw animation:** `CardHand.svelte` detects newly added card IDs via an internal `$effect` (comparing current vs previous hand state using `untrack()` to avoid reactive loops) and applies a `.card-drawn-in` class that triggers a `cardDrawnIn` keyframe — cards fly in from above when added to hand (e.g. after picker pick). **Enhanced draw animation (2026-04-12 — Bug B):** The `cardDrawnIn` keyframe was enhanced to add a gold burst glow: starts at `scale(0.55) translateY(40px)` with `filter: brightness(2.5) drop-shadow(0 0 12px rgba(250,204,21,0.9))` (bright gold flash), transitions to normal at 100% (`brightness(1) drop-shadow(0 0 0px transparent)`). This makes newly drawn cards clearly distinguishable from repositioned cards (which only slide via CSS `transform` transition). The existing `{#each cards as card, i (card.id)}` nanoid keying already prevented hand index shuffles — the enhanced animation addresses the playtest finding that repositioning looked like card replacement. |
| `CardCombatOverlay.svelte` | Root combat screen: wraps CardHand + QuizOverlay, handles surge/boss phases, landscape/portrait. **Dev skip button:** The `{#if $devMode}` block for `.dev-skip-btn` in `CardApp.svelte` is commented out as of 2026-04-12 — button no longer renders even in dev mode. (Was: positioned fixed below MusicWidget, calling `devForceEncounterVictory()`.) Computes `damagePreviews` via `damagePreviewService` and passes to CardHand. Derives `activeChainColor = turnState?.activeChainColor ?? null` and passes to both ChainCounter and CardHand (AR-310). Calls `ambientAudio.duck()` / `unduck()` via `$effect` reactive on `isQuizPanelVisible` — ducks ambient when charge quiz panel is showing. **AP display (2026-04-09 visual polish):** `.player-ap-right` is the sole AP display — the `.ap-label` "AP" span has been removed, leaving only `.ap-num`. Background upgraded to amber: `rgba(180, 100, 40, 0.85)` (portrait) / `rgba(208px, var(--layout-scale))` (landscape; shifted from `left: 148px` to `left: 208px` to give more clearance from End Turn). Active state: `rgba(220, 140, 60, 0.92)` with amber glow. `.ap-num` color: `#fff7e6`. **End Turn button (2026-04-09):** Background upgraded from `#854d0e` to `#b8860b` (brighter gold); pulse keyframe colors updated to `#b8860b`/`#c4960d`; portrait `right` offset bumped from `70px` to `90px` to match. **Enemy intent bubble (2026-04-09 damage formula fix):** `intentDetailText` and `intentDisplay` now show the actual runtime damage via `displayDmg(base, enemy)` — mirrors `enemyManager.executeEnemyIntent()` formula: `Math.round(base × strengthMod × floorScaling × GLOBAL_ENEMY_DAMAGE_MULTIPLIER)`. Imports added: `getFloorDamageScaling` from `enemyManager`, `GLOBAL_ENEMY_DAMAGE_MULTIPLIER` from `balance`. Attack intent copy changed from "Charging N damage next turn" to "Attacking for N damage" — the word "Charging" is now reserved for the `charge` intent type. Multi-attack: "Attacking for N × M hits". **Enemy intent bubble — HP damage after block (Issue 11, 2026-04-11):** `displayImpact(intent, enemy)` replaces the old `displayDmg(base, enemy)` helper. It calls `computeIntentHpImpact()` with `playerBlock = turnState.playerState.shield` and `act` derived from `currentFloor` (≤6 → Act 1, ≤12 → Act 2, 13+ → Act 3), returning `{ raw, postDecayBlock, hpDamage }`. Intent detail line shows: "Attacking for N HP damage" (no block), "N HP damage (raw − postDecayBlock block)" (partial block), or "Fully blocked (raw absorbed)" (fully blocked). Fully-blocked bubble gets `.intent-bubble-blocked` CSS class (muted slate text) and `data-intent-blocked="true"` attribute. `computeIntentDisplayDamage` import replaced with `computeIntentHpImpact, IntentHpImpact`. `EnemyIntent` type added to enemies import. **Co-op cancel (2026-04-09, updated Issue 9 2026-04-11):** The End Turn button transforms into a three-state control when `coopWaitingForPartner` is true: (1) **CANCEL END TURN** (amber `.cancel-state` class, enabled) when hand is non-empty — calls `handleCancelEndTurn()` → `cancelEndTurnRequested()` from `encounterBridge`; (2) **WAITING…** (disabled, `title` tooltip: "You played all your cards — waiting for your partners") when hand is empty; (3) normal **END TURN** when not waiting. The `.coop-waiting-banner` with `.coop-cancel-btn` also remains as a secondary path — `handleCoopCancel()` now routes through `cancelEndTurnRequested()` instead of calling `cancelCoopTurnEnd()` directly. `handHasCards = $derived(handCards.length > 0)` gates state 1 vs 2. `.end-turn-btn.cancel-state` CSS: `background: #92400e`, `color: #fcd34d`, `min-width: calc(160px * var(--layout-scale, 1))`, `cancelPulse` animation. `DamagePreviewContext` includes `playerStrengthModifier: getStrengthModifier(ps.statusEffects)` — damage previews show red/green when player has weakness/strength. **`onplaycard` signature (2026-04-04):** Last parameter `wasQuizzed?: boolean` added. **Wow-factor fixes (2026-04-08):** `showWowFactor` signature changed to `showWowFactor(answeredFactId: string, card: Card)` — takes the fact ID actually shown in the quiz instead of `card.factId`, fixing study-mode cases where the quizzed fact differs from the card base. Call site derives `answeredFactId = card.__studyFactId ?? card.factId`. Display duration doubled: timers changed from `turboDelay(5200)/5500` to `turboDelay(10200)/10500` (hold 5s → 10s). **WowFactor popup click-to-dismiss (2026-04-12):** Popup is now clickable — `pointer-events: auto; cursor: pointer` on the wow-factor container; `onclick` handler triggers 200ms fade-out animation then clears the popup state. Auto-dismiss at 10s still works. **Z-index layering (2026-04-09):** `.player-ap-right` uses `z-index: 26` in both portrait and landscape (raised from 6/20) — sits above `.quiz-backdrop` (25) so AP indicator stays fully visible at full brightness during quiz. `.end-turn-btn` uses `z-index: 26` in both portrait (raised from 5) and landscape (raised from 20) for the same reason. Both are covered by `CardExpanded.svelte` (`z-index: 30`) so the quiz window renders on top. The quiz backdrop (`z-index: 25`) dims everything below it including MultiplayerHUD and ChainCounter. **AP badge vertical position (Issue 3, 2026-04-11):** `.layout-landscape .player-ap-right` `bottom` raised from `calc(16px * var(--layout-scale, 1))` to `calc(22px * var(--layout-scale, 1))` (+6px) — aligns badge better with the End Turn button row. **Transmute pick → draw handoff (Issue 5, 2026-04-11):** `handleCardPickSelect` now captures the return value of `resolveTransmutePick()` as `pickResolvedCards: Card[]` and calls `drawHand(turnState.deck, pickResolvedCards.length)` once — this triggers the normal draw animation so picked cards fly from the draw pile into hand. Previously the cards were staged in `drawPile` but never drawn (stuck). `drawHand` imported from `../../services/deckManager` alongside existing `reshuffleEvent` import. **Bug 3 fix (2026-04-12 — intent display pin):** `displayImpact(intent, enemy)` now reads `enemy.lockedDisplayDamage` (pinned by `turnManager.ts` at intent-roll time) as the `raw` value when available, then applies block decay via the same formula as `computeIntentHpImpact()` without re-calling `computeIntentDisplayDamage()`. This stops the intent number from drifting when the player plays Block cards mid-turn. Falls back to the full `computeIntentHpImpact()` call when `lockedDisplayDamage === undefined` (pre-fix saves). Imports added: `BLOCK_DECAY_PER_ACT`, `BLOCK_DECAY_RETAIN_RATE` from balance. **Enemy intent weapon art (2026-04-13):** Intent bubble uses `getIntentIconPath(intentType, { statusEffect, value, hitCount })` from `iconAssets.ts` to pick the intent image. Attack intents are enriched by metadata: poison → venom_fang, burn → arc_scroll, bleed → hunter_bow, weakness/vulnerable → hex_totem, damage > 10 → war_axe, damage ≤ 4 → rust_dagger; default attack → cardtype_attack. Non-attack types use fixed paths (multi_attack → twin_blades, defend → round_shield, buff → surge_sigil, debuff → snare_thread, heal → tool_kit, charge → comet_shard). Emoji fallback via `INTENT_EMOJI` shown in `.intent-icon-fallback` (hidden by default). **Bug 4 landscape chain indicator (2026-04-12):** The `.lsb-chain-indicator` (right of card hand in landscape) now shows the chain type name via `getChainTypeName(chainType)` instead of the generic label 'Chain'. The `.lsb-chain-count` span is wrapped in `{#key chainMultiplier}` so it remounts on each multiplier increase, firing the `lsbChainPop` CSS animation (300ms cubic-bezier spring, same as `ChainCounter`). Both respect `@media (prefers-reduced-motion: reduce)`. **Bug C fix (2026-04-12 — mastery popup reads live store):** `triggerMasteryFeedback()` was checking `turnState` (the component prop) immediately after `onplaycard()` returned. Svelte 5's reactive scheduler batches prop updates asynchronously, so the prop still reflected the pre-play state. Fix: both the downgrade check (wrong answer path) and upgrade check (correct answer path) now call `get(activeTurnState)` directly (Svelte's synchronous store reader) to get the post-play deck state, then look up `discardPile.find(c => c.id === cardId)` for `masteryChangedThisEncounter`. `activeTurnState` added to imports from `encounterBridge`. **Enemy intent bubble landscape offset (2026-04-12):** Normal state left shifted from `50%` to `39%`; quiz-active state shifted from `72%` to `61%` — moves bubble leftward so it no longer overlaps the enemy sprite center. **Enemy power badges landscape (2026-04-12):** Landscape overrides in `CardCombatOverlay.svelte` pin badges to `top: 14vh; left: 76%` (aligned with Phaser HP bar at `LANDSCAPE.ENEMY_HP_Y_PCT=0.14`); quiz-active shifts to `left: 96%`. Badge icons shrink to 32px. See `EnemyPowerBadges.svelte` row for full details. **Combined buff+attack intent display (2026-04-18):** When `EnemyInstance.buffFollowUpIntent` is set by game-logic (enemy will buff then immediately attack), the intent bubble shows a two-row merged display: a subdued `.intent-buff-prefix` row (buff icon + buff telegraph text at 75% opacity) above the normal attack row. The bubble uses attack colors (red-ish) since the attack is the primary threat. `followUpIntent = $derived(turnState?.enemy?.buffFollowUpIntent ?? null)` drives the display change. `intentDisplay` uses `enemy.lockedFollowUpDisplayDamage` (pinned at intent-roll time by game-logic, same pattern as `lockedDisplayDamage`) for the follow-up damage value. `intentDetailText` shows "Buffs {statusEffect.type}, then attacks for {damage} damage". New CSS classes: `.intent-buff-prefix` (flex row, 13px font, 75% opacity), `.intent-prefix-icon` (20px, pixelated), `.intent-prefix-text` (weight 600, 80% white). All sizing uses `calc(Npx * var(--layout-scale/text-scale, 1))`. |
| `CombatHUD.svelte` | Legacy HP bars + combat log; largely superseded by InRunTopBar |
| `InRunTopBar.svelte` | Landscape/Portrait HUD: HP bar, shield badge, gold, floor/segment, relic tray, fog level, pause. Inline player status effect icons (poison, burn, etc.) to the right of the HP bar — hover to show per-icon popup. `.hp-group` uses `flex: 1 0 auto` (never shrinks when icons are added). Icon size matches HP bar height (`var(--topbar-height) * 0.58`). `.topbar-status-icon` has `background: transparent` and no `border-radius` — icons sit free-floating against the topbar (no dark oval), with a single tight dark outline `drop-shadow(0 0 1px rgba(0,0,0,0.9))` for contrast (2026-04-09: replaced white double glow which washed out small icons). `.topbar-status-sprite` and `.topbar-status-popup-sprite` use browser-default bilinear filtering (no `image-rendering` override). `.topbar-status-popup-sprite` is `28px` (scaled). Status popup is per-wrapper positioned below each icon on hover; backdrop click pattern removed. `.section-left` uses `max-width: 35%`. Accepts optional `statusEffects` prop (from `topBarPlayerEffects` derived in CardApp.svelte). **Icon paths (2026-04-09):** Status icons that exist in `sprites-hires/` use those paths (poison, weakness, vulnerable, strength, regen, immunity) — sharper at small render sizes. burn and bleed remain on the lower-res path until hi-res assets are generated. **Badge consolidation (2026-04-09):** Two separate badges (`.topbar-status-stack` value > 1 at top-right, `.topbar-status-turns` always at bottom) collapsed into ONE `.topbar-status-stack` badge at bottom-right showing `value` whenever `value >= 1`. Turns remaining moved exclusively to the hover popup. Badge has dark border (`rgba(0,0,0,0.5)`) and text-shadow for readability against any icon. **Fog gauge:** `getAuraLevel()` returns 0-10. The gauge maps this to a -5..+5 display range via `{@const displayFl = fl - 5}` — negative = flow (green, fills left), positive = fog (red, fills right), zero = neutral (2026-04-01 fix). **Button sizing (2026-04-04 fix):** `.pause-btn` height is `var(--topbar-height) * 0.7` (reduced from 0.85) so it fits within the topbar with centering margin and no viewport clip; `max-height: 100%` removed (was ineffective). `.relics-row` has `max-height: 100%` to prevent relic buttons from exceeding container height. **2026-04-08 HUD fix (task 5.2):** `.relic-slot-wrapper` wrapper div removed — relic `<button>` and tooltip are now direct children of `.relics-row`. The tooltip `<div>` is a sibling of the button, not nested inside it. `.relics-row` has `position: relative` added to support tooltip absolute positioning. **Fog wing wrapper height (2026-04-10):** `.fog-wing-wrapper` now has explicit `height: calc(28px * var(--layout-scale, 1))` matching its `.fog-wing` child — prevents the hitbox from extending below the visual element (which would overlap the `MultiplayerHUD` below it). **2026-04-09 pill removal:** The `reviewQueueLength` prop and `{#if reviewQueueLength > 0}` pill block were removed — the review queue exists (used by `Recall` card and `Scholar's Crown` relic) but is no longer surfaced in the top bar UI. The `.review-queue-pill` CSS class has been deleted. **Deck viewer button (2026-04-11 Issue 14):** `.deck-btn` added immediately before `.pause-btn` in the right section. Calls `openRunDeckOverlay()` from `runDeckOverlayStore.ts`. Both `.deck-btn` and `.pause-btn` now have `align-self: center` and `min-width/min-height: calc(44px * var(--layout-scale, 1))` fixing the pre-existing gear icon centering issue. **Stacked-card deck icon (2026-04-11):** The 🎴 emoji in `.deck-btn` replaced with a `.deck-stack-icon` container holding a `.deck-stack-cards` div of `.deck-stack-card` stacked rectangles (2px offset per layer, white border, gold-tinted top card). Layer count = `Math.max(1, Math.min(5, Math.ceil(totalCards / 8)))` — 1 rect for ≤8 cards, 5 for 40+. A `.deck-stack-count` badge overlaid on the bottom-right corner of the top card rect shows the live total card count (sum of all four piles). The badge uses `position: absolute; bottom: calc(2px * var(--layout-scale, 1)); right: calc(2px * var(--layout-scale, 1))` inside `.deck-stack-icon` (`position: relative`) — same pattern as the Slay-the-Spire draw pile indicator. Dark semi-transparent background (`rgba(10, 18, 30, 0.85)`) and `border-radius: calc(3px * var(--layout-scale, 1))` keep the number readable against the card rect. **2026-04-11 fix:** Previously rendered as a flex column child below the stack, clipping at the 49px topbar bottom edge. Card count sourced from `$activeTurnState` via the `encounterBridge` import — shows 0 when outside combat. `.deck-btn` has `margin-left: calc(14px * var(--layout-scale, 1))` (widened from 4px) to create a visible gap between the relic row and the deck button. **SVG gear icon (2026-04-11):** The ⚙ emoji in `.pause-btn` replaced with an inline `<svg class="pause-gear-svg">` — a standard two-path gear design with `stroke="currentColor"` that inherits the button's `rgba(255,255,255,0.75)` color. Size: `calc(20px * var(--layout-scale, 1))`. Eliminates glyph-bearing drift that caused the ⚙ emoji to misalign vertically relative to the deck icon. `.pause-btn` `margin-left` tightened from `calc(4px)` to `calc(2px)` so the gear sits flush next to the deck stack (visually grouped pair). **Bug 2 fix (2026-04-12 — sentinel gate):** Imports `PERMANENT_DURATION_SENTINEL` from `../../data/statusEffects`. Status effect popup uses `topbarDescForEffect()` helper (sentinel-aware: patches `(9999 turn...` → `(permanent)` and bare `9999` → `∞`). Aria-label on status icon buttons now reads 'permanent' instead of '9999 turns' for sentinel-valued effects. **Text size increases (2026-04-12):** `.segment-name` 14→21px, `.floor-label` 13→20px, `.progress-divider` 10→15px, `.gold-value` 16→24px, `.gold-icon` 18→24px. **Deck button resized (2026-04-12):** Width multiplier 0.65→0.585, height 0.7→0.63, `min-width/min-height` 44→40px, icon 28×36→25×32px — 10% smaller for better topbar proportions. **Relic tooltip enlarged (2026-04-12):** `.tooltip-name` and `.tooltip-desc` 9→14px, container `max-width` 200→300px, `min-width` 130→180px. Added `max-height: 80vh; overflow-y: auto` for screen bounds clamping on small viewports. |
| `MusicWidget.svelte` | Ultra-sleek Spotify-style BGM player widget. Single expanding container: collapsed pill (~56×36px, top-right, `position: fixed`, z-index 201) expands to a 240px glass panel — not two separate elements. Collapsed shows a 6-bar spectrogram canvas (48×32) built from AnalyserNode frequency bins. Expanded shows: wider spectrogram, track title + marquee overflow, **EPIC / LO-FI** category toggle, prev/play-pause/next controls, volume slider with mute toggle, ambient on/off toggle (`.ambient-row` with 'AMB' label + speaker SVG button; green when active, dim when off — synced with `ambientEnabled` store from `cardAudioManager`). The SHOP jukebox button has been removed — track purchases are now in the **Jukebox tab** of `CampUpgradeModal`. Volume and mute synced with `cardAudioManager` `musicVolume`/`musicEnabled` stores (single source of truth, not internal widget state). **Mute button (2026-04-10):** Calls `musicService.toggleMute()` — not `musicEnabled.update()` directly. `toggleMute()` calls `init()` first to ensure store subscriptions are active, then applies volume immediately to `currentAudio`. rAF animation loop driven by `musicService.getFrequencyData()` — idle sine wave when not playing. Glass morphism: `backdrop-filter: blur(20px)`, `rgba(8, 10, 18, 0.4)` background. All sizing uses `clamp()` units (viewport-clamped). **Volume slider (2026-04-14):** Calls `musicService.init()` before setting volume to ensure store subscriptions are active (same pattern as mute). **Panel collapse fix (2026-04-14):** Expanded panel no longer collapses when clicking inner controls (volume slider, category toggle, playback buttons) — panel only closes via click-outside. Closes on outside click. Rendered in `CardApp.svelte` alongside `InRunTopBar` whenever `showTopBar` is true. `musicService.startIfNotPlaying()` auto-fires on entering any `IN_RUN_SCREENS`. |
| `ChainCounter.svelte` | Animated chain streak badge. **AR-310:** Always-visible active chain color bar (`.active-chain-bar`) is the **sole** chain UI element (2026-04-08 HUD fix — `.chain-display` removed). At chain length 0: shows colored dot + chain name + "Play to chain!" hint. At chain >= 1: shows multiplier in the active chain color. Multiplier color uses `activeChainColor` (the turn's chain type), NOT `chainType` (last played card) — task 7.1 fix. Tier escalation at chains 3/4/5: `.chain-tier-3/4/5` classes on `.active-chain-bar` animate the multiplier span. `.active-chain-bar` uses `z-index: 20` (below `.quiz-backdrop` at 25 — dims during quiz, re-emerges after). Props: `isPerfectTurn`, `chainLength`, `chainType`, `chainMultiplier`, `activeChainColor: number | null`. **Bug 4 fix (2026-04-12 — chain build pop):** `multKey` state increments whenever `chainMultiplier` increases. The `.chain-mult` span is wrapped in `{#key multKey}` so it remounts on each chain extension, triggering the `chainBuildPop` CSS animation (300ms cubic-bezier spring, 1.45x peak scale). This gives the player per-card visual feedback that their chain is growing and the multiplier just changed. `@media (prefers-reduced-motion: reduce)` disables the pop. **Chain-break animation (2026-04-12 — BATCH-2026-04-12-001):** When `chainMultiplier` drops from >1.0 back to 1.0 (off-color card breaks the chain), a `breakKey` state variable increments. The `.active-chain-bar` div is wrapped in `{#key breakKey}` (nested inside `{#key activeColorKey}`) — remounting triggers the `chainBreakShake` CSS keyframe: a 250ms horizontal shake with red-tinted `brightness`/`hue-rotate` filter that communicates the chain break clearly. `.active-chain-bar.chain-breaking { animation: chainBreakShake 250ms ease-out; }` The class is applied while `breakKey > 0`. `@media (prefers-reduced-motion: reduce)` disables the shake. |
...
| `DuelOpponentPanel.svelte` | **Wired 2026-04-22 (L-026, H-015).** Real-Time Duel opponent state panel — left-side overlay rendered during combat when `currentLobby.mode === 'duel'`. Replaces `MultiplayerHUD` for duel-mode runs. Props: `opponentName`, `opponentHp`, `opponentMaxHp`, `localDamageTotal`, `opponentDamageTotal`, `opponentChainLength`, `opponentChainColor`, `lastTurnSummary?`, `turnTimerSecs`, `turnTimerMax`, `enemyTargetIsLocal`. Currently fed by `opponentProgress.playerHp` from `multiplayerCoopSync` partner state; chain/damage/timer/target fields use safe defaults until duelService exposes per-turn aggregates to the UI layer. All sizing uses `calc(Npx * var(--layout-scale, 1))` / `--text-scale`. Render condition lives in `CardApp.svelte` next to the `MultiplayerHUD` block: `{#if isMultiplayerRun && !isManyPlayerMode && currentLobby?.mode === 'duel'}`. |
| `DamageNumber.svelte` | Floating combat numbers (damage, block, heal, poison, burn, bleed, gold, crit). Type color mapping: damage=`#FF4444` (red), block=`#4499FF` (blue), heal=`#44FF88` (green). The `onDamageNumber` callback in `juiceManager.ts` has signature `(value: string, isCritical: boolean, cardType?: string)` — `cardType` (3rd param, optional) is the card chain type string, available for callers that want to tint numbers by chain color. |
| `StatusEffectBar.svelte` | Row of active status effect icons + counts during combat. Enemy bar positioned at `14vh` (portrait) / `18vh` (landscape override via ). Player StatusEffectBar removed — player effects now inline in InRunTopBar. Icon `.effect-icon` container is `50px` (scaled). `.effect-sprite-icon` is `40px` (scaled); `.popup-sprite-icon` is `30px` (scaled). Both sprite icons use browser-default bilinear filtering (no `image-rendering` override) — avoids aliasing artifacts when downscaling 256px source art to small display sizes. Stack badge (`.effect-stack`) shows for `value >= 1` (not `> 1`), sized 20×20px. Turn counter (`.effect-turns`) font 12px, color `#e2e8f0`, with text-shadow. `brain_fog` and `flow_state` desc functions show context-aware text based on fog level thresholds: ≤2 = Flow State active, 3-6 = Neutral, ≥7 = Brain Fog active. **Bug 2 fix (2026-04-12 — sentinel gate):** Imports `PERMANENT_DURATION_SENTINEL` from `statusEffects.ts`. `.effect-turns` badge now calls `turnsLabel(t)` which returns `∞` when `t >= PERMANENT_DURATION_SENTINEL` instead of the raw 9999 number. Permanent badges get `.effect-turns-permanent` class (gold color, slightly larger font). Popup desc uses `descForEffect()` which regex-patches any `(9999 turn...` fragments to `(permanent)` and remaining `9999` occurrences to `∞`. Aria-label uses `isPermanent ? 'permanent' : N turns`. **Popup enlarged (2026-04-12):** `.popup-name` 12→18px, `.popup-desc` 11→16px, `.popup-icon` sprite 16→24px inside popup. Container width 240→340px with `max-width: min(340px, 90vw)` clamp to viewport. Sprite icons in popup: 30→40px. **Sprite icon upgrades (2026-04-13):** focus, fortify, double_strike, hardcover, foresight, and overclock upgraded from emoji fallback to dedicated sprite icons — each uses a `spriteIcon` path pointing to a reward icon in `/assets/sprites/icons/`. Emoji fallback (`.effect-emoji`) still renders when `spriteIcon` is absent. |
...
| `EnemyPowerBadges.svelte` | Badges showing elite/boss modifier tags on an enemy. **Portrait:** `.badge-icon` is `60px` (scaled), container `position: fixed; top: 22vh; left: 50%; transform: translateX(-50%)`. **Landscape (2026-04-12):** `:global(.layout-landscape .enemy-power-badges)` overrides to `position: fixed; top: 14vh; left: 76%; transform: none` — vertically centered with the Phaser enemy HP bar (`LANDSCAPE.ENEMY_HP_Y_PCT = 0.14`). Quiz-active shifts badges to `left: 96%` (off-screen-right) via `:global(.layout-landscape.quiz-active .enemy-power-badges)`. `.badge-icon` shrinks to `32px` in landscape (from 60px). Both landscape overrides live in `CardCombatOverlay.svelte` after the status-icon-wrapper block. **2026-04-08 HUD fix:** Repositioned from centered under sprite (fixed position) to right of HP bar. **2026-04-09 positioning fix:** Base portrait style set to `top: 22vh; left: 50%; transform: translateX(-50%)`. **`enemyPowers.ts` tooltip text (updated 2026-04-12):** `punish_wrong` → "Wrong answers on Charged cards trigger a penalty"; `punish_correct` → "Correct Charges trigger a reaction"; `punish_skip` → "Gains Strength if you skip Charge for a turn"; `escalates` → "Gains Strength each turn". |
...
| `RunDeckOverlay.svelte` | **Full-run deck viewer (2026-04-11, updated fixes).** Modal overlay showing ALL cards in the run as a responsive `CardVisual` grid (Slay-the-Spire style). **Grid layout:** `display: grid; grid-template-columns: repeat(auto-fill, minmax(calc(170px * var(--layout-scale, 1)), 1fr))` — auto-fills the panel width with ~170px card columns. Each card renders inside a `.rdo-card-visual-wrapper` (position:relative, sized for CardVisual's `inset:0` contract) with `--card-w` CSS var set for correct CardVisual typography scaling. Cards are wrapped in `.rdo-card-cell-btn` (`<button>` type=button) which holds the CardVisual wrapper plus two absolute-positioned badges: `.rdo-card-pile-badge` (floats ABOVE card top, horizontally centered via `top: calc(-18px * var(--layout-scale, 1)); left: 50%; transform: translateX(-50%)`, color-coded per pile, `pointer-events: none`) and `.rdo-card-mastery-badge` (bottom-right, shown only when masteryLevel > 0). Grid has `padding-top: calc(22px * var(--layout-scale, 1))` so floating badges are not clipped. **Mastery popup (2026-04-11):** Clicking a card opens a `position: fixed` translucent popup (`z-index: 460`, dark bg + gold border + backdrop-filter blur) showing the mechanic name header and six rows (L0–L5). Each row shows the mastery upgrade icon, the level label (color-coded by masteryColor), and the description built by `getCardDescriptionParts(cardAtLevel(card, level))` with same part-type rendering as CardVisual. **Icon rendering (2026-04-12):** The upgrade icon is a `<div class="rdo-popup-icon">` using `background-image` + `background-size` + `background-position` to CSS-crop the 73×73 plus region out of the 886×1142 `card-upgrade-icon.webp` canvas — the earlier `<img object-fit:contain>` path shrank the plus to ~2px because most of the canvas is transparent padding. Icon display size is 40px (scale factor 40/73 ≈ 0.548, scaled canvas ~485×626, plus offset ~(-76,-294)). Per-level color comes from a `--rdo-icon-filter` inline CSS var: `getMasteryIconFilter(level)` for L1–L5 and `grayscale(1) brightness(0.55) opacity(0.6)` for L0. The current-level row gets `.rdo-popup-level-current` (gold-tinted bg + border) and its icon has the `.rdo-popup-icon-bob` animation (4px bob, 1.6s loop). Popup opens to the right of the card; flips left if near right viewport edge. Clamped to stay on screen. Closes on: X button, Escape key (first Escape closes popup, second closes overlay), clicking another card, grid scroll, or backdrop click. State: `let selectedCard = $state<{card, pile, top, left} | null>(null)`. **Sort/filter controls:** Filter (All / Hand / Draw / Discard / Forget) and Sort (Pile / Type / A-Z / Mastery). **Panel width:** `calc(900px * var(--layout-scale, 1))`. Opened via deck icon in `InRunTopBar`. Reads `$activeTurnState.deck`. Empty state with escape button (softlock prevention). `data-testid="run-deck-overlay"`. z-index: 400. |
...
### Sync-health banner (MP-STEAM-20260422-009, added 2026-04-22)

`CardApp.svelte` renders a transient top-center banner when a coop partner stops sending heartbeats. Hooked via `onPartnerUnresponsive` from `multiplayerCoopSync`. The banner uses `data-testid="sync-health-banner"`, an amber pulsing dot, the partner display name from `currentLobby.players`, and an explicit dismiss button. Auto-clears after 8s. Reuses the existing `_lastPartnerHeartbeat` tracker in `multiplayerCoopSync` — no new heartbeat machinery added here. CSS: `.sync-health-banner` (fixed top, z-index 9000, all sizing via `calc(Npx * var(--layout-scale, 1))`).
...
| `StudyQuizOverlay.svelte` | Rest-room study quiz: boss-quiz–style questions to upgrade card charges. Shows inline `SRS +` / `SRS -` indicator (green/red, 0.65 opacity, scaled `10px`) alongside correct/wrong feedback text. **Japanese grammar rendering (2026-04-08):** When the current question carries `sentenceFurigana` segments, renders via `GrammarSentenceFurigana.svelte` with furigana/kana-only/hover support. Reactively shows `sentenceTranslation` (italic muted) and `sentenceRomaji` (when romaji toggle ON). Always shows `grammarPointLabel` as a teal-bordered hint label below the translation. Grammar rendering is MCQ-only in study mode (no typing path). CSS classes: `.grammar-fill-blank`, `.grammar-translation`, `.grammar-romaji`, `.grammar-hint-label`. **Chess puzzle integration (2026-04-10):** When `question.quizResponseMode === 'chess_move'` and `fenPosition`/`solutionMoves` are set, renders `<ChessBoard>` instead of answer buttons. `$effect` rebuilds `chessContext` (via `getPlayerContext()` from `chessGrader`) whenever the question changes. `handleChessMove(uci)` grades the move, optionally updates chess Elo via `updateChessElo()`, then calls `selectAnswer()` after 400ms (correct) or 800ms (wrong). Board is disabled after the player moves or while feedback is showing. Falls back to standard MCQ if `getPlayerContext()` throws (invalid puzzle data). `showNotationInput={$isLandscape}` passed to ChessBoard. CSS: `.chess-puzzle-container` flex column, centered. **Numerical answer display fix (2026-04-12):** All answer-button render sites now wrap answer text in `displayAnswer()` from `numericalDistractorService` — strips `{N}` brace markers so numeric facts (e.g. `{206}`) show as `206` instead of leaking raw bracket notation into button labels. Import: `import { displayAnswer } from '../../services/numericalDistractorService'`. Three render sites fixed: `.answers-grid` text buttons, `.answers-image-grid` `aria-label`, and `.study-image-label` span. Same fix applied to `ShopRoomOverlay.svelte` haggle quiz buttons. **Map pin integration (2026-04-12):** When `question.quizResponseMode === 'map_pin'` and `mapCoordinates` is set, renders `<MapPinDrop>` instead of answer buttons — world capitals deck now shows the interactive world map in study mode instead of falling through to plain MCQ. Imports `MapPinDrop` and `updateGeoElo, tierToRating` from `geoEloService`. State: `mapDisabled` (locked after confirm), `mapEloChange` (shows Elo delta badge via `.elo-change-badge`/`.elo-positive`/`.elo-negative` CSS — same pattern as `CardExpanded`). `handleMapPinConfirm(pinCoordinates, distanceKm, accuracy)` updates Geo Elo, then calls `selectAnswer(correctAnswer)` after 400ms when `accuracy >= 0.5` or `selectAnswer(wrongAnswer)` after 800ms. `mapDisabled` and `mapEloChange` reset in `selectAnswer`'s advance-to-next timeout alongside the existing chess state reset. Container: `.map-pin-quiz-container` (flex column, `min-height: calc(300px * var(--layout-scale, 1))`, landscape override: `360px`). **Always-write typing mode (2026-04-13):** New prop `quizLanguageCode?: string | null`. When `$deckOptions[quizLanguageCode]?.alwaysWrite` is true, renders `GrammarTypingInput` (Japanese) or `TypingInput` (all other languages) instead of MCQ buttons. Exclusions: image_question, image_answers, chess_tactic, chess_move, map_pin, numerical answers (`isNumericalAnswer()`), answers >80 chars. After submit, `showFeedback` becomes true and falls through to the MCQ grid for correct/wrong answer highlighting (matches `CardExpanded` behavior). `CardApp.svelte` passes `quizLanguageCode={studyLanguageCode}`; `studyLanguageCode` is a new `$state` set in `handleRestStudy()` from `activeDeckMode` (handles `study.deckId`, `preset.presetId`, `language.languageCode` variants) and cleared in `handleStudyComplete()`. Scenario injection also sets `Symbol.for("rr:scenarioStudyLanguage")` in `scenarioSimulator.ts` alongside existing `rr:scenarioStudyQuestions`. |
...
| `CardUpgradeRevealOverlay.svelte` | Card upgrade reveal popup shown on screen `cardUpgradeReveal`. Displays the before and after version of a card side by side with an animated arrow, a stat diff strip (mastery level, damage/QP, block, AP cost, new tags), and a Continue button. Props: `beforeCard`, `afterCard`, `mechanicName`, `beforeLevel`, `afterLevel`, `ondismiss`. Mounted in `CardApp.svelte` — gated by `$currentScreen === 'cardUpgradeReveal' && $activeCardUpgradeReveal`. Reads `getMasteryStats()` and `getEffectiveApCost()` from `cardUpgradeService`. Plays `event-positive` on mount. z-index: 220. Card dimensions: 35vh tall, 886/1142 aspect ratio (matches CardPickerOverlay). Animations: overlay fade-in, before-card slides left, after-card slides right with gold glow, arrow pulses. Backdrop click and Escape/Enter/Space all dismiss. |
...
| `grammarPointLabel` | `string` | Short label (e.g., "が — subject marker particle"); shown AFTER answering (when `waitingForGotIt` is true) with a fade-in reveal — NOT before, to prevent spoilers |

The `showRomaji` reactive is derived from `$deckOptions?.ja?.romaji`. A `.grammar-typing-hints` panel (teal left-border box) renders above `GrammarTypingInput` when typing mode is active and `sentenceTranslation` is present (only the translation hint, not `grammarPointLabel`). Grammar rendering no longer loops `sentence.split('{___}')` — `GrammarSentenceFurigana` handles blanks internally via `{ t: "{___}" }` segment entries.

**Grammar point spoiler prevention (2026-04-13):** `grammarPointLabel` is no longer shown in the `.grammar-typing-hints` pre-answer block — it was revealing the answer (e.g. "ている — to do something for someone"). It is now shown post-answer inside the `waitingForGotIt` block with a 250ms `.grammar-point-label-reveal` fade-in animation.

**Duplicate sentenceTranslation removed (2026-04-13):** The `.grammar-typing-hints` block inside the answer zone that re-displayed `sentenceTranslation` has been removed. `sentenceTranslation` is already shown under the question in the question zone; the duplicate created confusing repeated text during Japanese grammar typing quizzes.

**Manual-mode correct-answer continue overlay (2026-04-13):** When `$autoResumeAfterAnswer` is `false`, correct answers no longer auto-dismiss after 1600ms. Instead, after 500ms (time for the correct-answer animation), a `waitingForCorrectContinue = true` flag appears. This renders a transparent `.continue-overlay` button covering the full quiz panel with a pulsing "Click to continue" hint at the bottom. Clicking anywhere on the panel calls `onanswer(...)` and dismisses the quiz. Wrong answers continue using the existing `waitingForGotIt` / "Continue" button flow.
...
### CardExpanded Landscape Panel — Two-Zone Layout (2026-04-12)
...
- `top: calc(var(--topbar-height, 4.5vh) + calc(40px * var(--layout-scale, 1)))` — topbar height + fog meter (28px) + 12px gap ≈ 89px at 1080p.
...
- Content is split into two wrapper `<div>`s:
  - `.question-zone` — `flex: 1 1 0%` (was `1 1 auto`), `min-height: 0` ensures flex shrink works. Centers question vertically with `justify-content: center`. Holds question image, quiz-asset image, and `.card-question` div.
  - `.answer-zone` — `flex: 0 1 auto`, `min-height: 0`, `overflow-y: auto` (was `overflow: hidden`). Shrinks to content rather than forcing a floor. Holds map/chess/typing/text answer modes.
...
- `.card-question` in landscape: `overflow-wrap: break-word; word-break: break-word` added to prevent long words from overflowing horizontally.
- `.question-image` in landscape: `max-height: 30vh` cap added to prevent inline images from blowing out the container.
...
| Class | Trigger |
|-------|---------|
| `answers-tf-pair` | count=2 AND all answers in `["True","False","Yes","No"]` |
| `answers-two-wide` | count=2 AND maxLen < 30 |
...
- **Landscape overrides** (`.card-expanded-landscape .card-question.quiz-text-*`):
  - `quiz-text-short`: `42px`
  - `quiz-text-medium`: `34px`
  - `quiz-text-long`: `28px`
  - `quiz-text-extra-long`: `23px`
  - `quiz-text-max-long`: `19px`
...
- **Typing input (2026-04-13):** `.typing-input` padding: `14/16px` (was `10/12px`), font-size: `22px` (was `16px`), `text-align: center` (centers placeholder and typed text). `.typing-submit-btn` padding: `14/20px`, font-size: `22px`. Applied in both `GrammarTypingInput.svelte` and `TypingInput.svelte`.

**Image quiz in landscape:** `.quiz-asset-image` max-height capped at `40vh` inside landscape panel.
...
| `HubGlowCanvas.svelte` | Two-canvas hub glow system (updated 2026-04-01 perf pass, Windows perf fix 2026-04-20): (1) `position: fixed` canvas with `mix-blend-mode: screen` for additive warm orange radial glow; (2) sibling `<canvas class="hub-vignette-canvas">` with `mix-blend-mode: normal` for vignette darkening. Props: `campfireCenterFn: () => {x, y}` (absolute viewport pixels), `zIndex?: number` (default 1; vignette canvas gets `zIndex + 1`), `mouseX?: number` / `mouseY?: number` — forwarded to `HubGlowEffect.setMousePosition()`. **0.5× backing store (2026-04-20):** both canvases are sized at `Math.round(window.innerWidth * 0.5) × Math.round(window.innerHeight * 0.5)` via `syncSize()` while CSS `inset: 0` stretches them to fill the full viewport. Radial gradients upscale imperceptibly — this halves texture-upload cost (~8 MB → ~2 MB per canvas on a 1920×1080 display) and eliminates the Chromium/Windows compositor bottleneck caused by a full-viewport `mix-blend-mode: screen` canvas at native resolution. `BACKING_SCALE = 0.5` constant in the script. |
| `HubFireflies.svelte` | Ambient firefly particles driven by **shared-loop sine-wave motion** (updated 2026-04-01 perf pass, spawn-position fix 2026-04-20). Uses `hubAnimationLoop.ts` shared 30fps loop instead of own RAF. Spawns 15 fireflies on mount; keeps count stable by respawning dead ones immediately. **Motion:** each firefly has per-fly `phase`, `ampX`/`ampY`, `freqX`/`freqY`, `depthLayer` (0.5–1.0). Position updated every 30fps tick; Svelte `tick` $state only incremented every 3rd frame (~10fps) to reduce 15-element DOM style-recalc cascade. **Lifecycle:** fadingIn 600ms → alive 4–8s → fadingOut 800ms → dead → respawn. **Alpha:** `maxAlpha = 0.4 + depthLayer * 0.4` (0.6–0.8). **No CSS keyframes** — all motion is JS-driven. **Spawn:** full-screen excluding campfire zone 40–60%x / 55–75%y. **Size:** `(size * depthLayer)px * var(--layout-scale, 1)`. **Reduce-motion:** static positions, no callback registration. **Spawn bug fix (2026-04-20):** `spawnFirefly()` now initializes `px` and `py` from `pos.x * _vw / 100` / `pos.y * _vh / 100` instead of `0`. Previously, newly-spawned fireflies rendered at `translate(0,0)` for their first frame — invisible on fast machines but visible as a corner clump on the laggy Windows path. Both the `i >= 3` stagger branch and the reduce-motion branch also re-derive `px`/`py` after explicitly reassigning `fly.x`/`fly.y`. |
...
- **Scale-aware coords (2026-04-20):** all viewport-space values (campfire center from `campfireCenterFn()`, mouse X/Y) are projected into canvas-space before drawing: `scaleX = canvas.width / window.innerWidth`, `scaleY = canvas.height / window.innerHeight`. The `diagonal` and `Math.max(w,h)` calculations already operate on canvas pixels and are automatically correct at the 0.5× backing resolution.
- **Quantize + frame-skip (2026-04-20):** intensity is quantized to 20 discrete steps (`Math.round(raw * 20) / 20`). A cache key encodes quantized intensity + campfire/mouse coords at 1-decimal precision. If the key matches `this.lastKey` from the previous frame, `drawFrame()` returns early — the previous pixels persist, which is identical to re-drawing the same frame. `lastKey` is reset to `''` in `start()` and `destroy()` so a fresh mount always draws at least once.
...
| `DungeonEntrance.svelte` | **Dead code (2026-04-15).** First-run onboarding screen removed — players now flow directly to `dungeonMap`. File kept for reference; not imported or mounted anywhere. |
...
| `CustomDeckViewModal.svelte` | Rich inspection modal for viewing and editing a custom deck (renamed from `PlaylistViewModal.svelte`, upgraded 2026-04-07; bug-fixed 2026-04-11). **Props:** `customDeck: CustomDeck`, `onClose`, `onRemoveItem(itemIndex: number)`, `onDeleteDeck`, `onRenameDeck(newName: string)`. **Features:** (1) **Aggregate stats header** — overall progress bar, total/seen/mastered counts; uses `getDeckProgress()` summed across all study items. (2) **Expandable accordion items** — each deck row shows name, mini 80px progress bar, mastery %, fact count, and a chevron (▸/▾) when expandable; sub-deck items (`item.subDeckId` set) are **never expandable** (sub-decks do not nest, and parent deck tags do not belong to a sub-deck row). Progress bar for sub-deck items uses `getSubDeckProgress(deckId, subDeckId)`; for full-deck items uses `getDeckProgress(deckId)`. (3) **Sub-deck drill-down** (in expanded panel, full-deck items only) — lists each sub-deck with its own mini progress bar and mastery % via `getSubDeckProgress()`. (4) **Exam tag chips** (in expanded panel, full-deck items only) — shows tag display labels (using `formatExamTag()` from `src/ui/utils/examTagDisplay.ts`) with fact counts via `getTagFactIds()`. Tags are display-only. **Bug fixed 2026-04-11:** expanded panel previously rendered the parent deck's full sub-deck list for sub-deck items (sibling sub-decks incorrectly appeared as children), and the per-item progress bar showed the parent deck's aggregate progress instead of the sub-deck's own progress. Both fixed by guarding on `!item.subDeckId`. **Inline name editing**, per-item remove buttons, delete-with-confirmation flow. Backdrop z-index 350. All sizing: `calc(Npx * var(--layout-scale, 1))` / `calc(Npx * var(--text-scale, 1))`. **Deck icon picker (2026-04-13):** Header row shows the current deck icon (`customDeck.icon`) as a `<img>` next to the deck name; clicking it toggles `iconPickerOpen`. When open, a 23-icon grid appears (`CUSTOM_DECK_ICONS` constant — synergy, hub, reward, currency, and level icons from `/assets/sprites/icons/`). Selecting an icon calls `onChangeIcon(iconPath)`; a "No icon" button calls `onChangeIcon(undefined)`. Selected icon highlighted with `.icon-picker-selected` class. |
...
| Component | Purpose |
|-----------|---------|
| `ShopRoomOverlay.svelte` | Shop: buy/sell cards and relics, card removal, card transform, haggle quiz. Calls `ambientAudio.setContext('shop')` on `$effect`. **Visual overhaul (2026-04-13):** Cards now render using `CardVisual.svelte` (pixel-perfect V2 layered frame, `--card-w` CSS var required) in a `.cards-grid`. Relics render as `.relic-float-card` articles: 64px circular icon (`relic-icon-circle`) with rarity-colored glow border, `@keyframes relic-float` bob animation. **Layout redesign (2026-04-14):** `.shop-main` is now `flex-direction: column`. Three sections stacked top-to-bottom: (1) `.relics-row` — horizontal flex row with `flex-wrap: wrap; justify-content: center` replacing the old 240px `.relics-column` left column; (2) `.cards-grid` — CSS `display: grid; grid-template-columns: repeat(4, auto)` producing a 2×4 layout for 8 cards (up from single horizontal row); (3) `.shop-bottom-bar` — unified bottom bar with `services-row` (Card Removal + Card Transform) and `Leave Shop` button as siblings, all centered. `SHOP_CARD_W` bumped 140→155px for readability. `leave-shop-btn` is now a normal `service-btn` sibling inside `.shop-bottom-bar` (no longer `position: fixed` outside the section). **Layout v3 (2026-04-14):** Side-by-side two-column layout. `.shop-main` is `flex-direction: row`. Left column: `.relics-panel` (~33% width) — dark floating panel (`rgba(13,17,23,0.85)` bg, subtle border, `border-radius`) with relics stacked vertically and `justify-content: center`. Right column: `.cards-area` (`flex: 1`, ~66%) — column flex container with `.cards-grid` (4-across CSS grid, vertically centered) and `.services-row` (Card Removal + Card Transform, `margin-top: calc(24px * var(--layout-scale, 1))`). `leave-shop-btn` moved to `position: fixed; bottom/right` outside the `<section>` element. Card price buttons (`card-price-buy`) enlarged to match relic buy button height: `min-height: calc(36px)`, `font-size: calc(13px)`, `padding: calc(6px) calc(16px)`. **Card Transform wired** — `service-transform` reads `shopInventory.transformCost`, opens transform picker modal, calls `ontransform(cardId, haggled)` prop → `gameFlowController.onShopTransform()` → populates `pendingTransformOptions` store → `showTransformOptions` overlay renders `CardVisual` grid of 3 replacement choices. **Relic slot cap feedback (2026-04-12):** When `isRelicSlotsFull(runRelics)` is true, RELICS section label shows red badge, buy buttons disabled. **Relic tooltip (2026-04-12):** `.tooltip-backdrop` is `pointer-events: none`; click-outside dismiss via `document pointerdown` `$effect`. **Leave-shop confirm dialog (2026-04-12):** Inline modal replaces `window.confirm()` (blocked in headless Chrome). **Haggle fail behavior (2026-04-08):** No price penalty on fail — item keeps original price, haggle button disabled. **Relic panel cleanup (2026-04-14):** Removed dark container background/border from `.relics-panel` — relics float transparently over shop background. Relic hover tooltip disabled (description already visible inline). Relic name font bumped 14→20px, description 11→22px. Service button labels/prices bumped 14→21px. Card effect text shop override removed — uses `CardVisual`'s own responsive sizing. |
| `RestRoomOverlay.svelte` | Rest room: heal HP, study to upgrade a card, meditate to remove one. Calls `ambientAudio.setContext('rest')` on `$effect`. **Meditate disabled message (2026-04-12):** `meditateDisabledReason` prop (passed from `CardApp.svelte`) updated from "Deck too small (min 5)" → "Need 6+ cards to remove one" to match the actual `canMeditate()` check (`length > 5`, requiring 6+ cards). |
| `MysteryEventOverlay.svelte` | Mystery event: narrative choice cards with quiz-gated outcomes. Calls `ambientAudio.setContext('mystery')` on `$effect` when event is set. **`buildStudyFacts()` trivia fallback (2026-04-08, task 13.2b):** When `deckMode.type === 'study'`, pulls 3 random facts from the curated deck (existing behavior). When in trivia dungeon mode (no study deck), falls back to `factsDB.getTriviaFacts()` — excludes language/vocab facts and picks 3 random from the trivia pool. Both branches shuffle then slice(0,3). **Transition flash fix (2026-04-14):** Added `transitionSettled` boolean state — overlay content is hidden (`opacity: 0`) until the `ParallaxTransition` `onSettle` callback fires, preventing a brief flash of event content before the room transition animation completes. **`freeCard` effect (2026-04-14):** Implemented in `gameFlowController.ts`. Picks a random card from the run pool and adds it to the player's deck, then shows a green-accented single-card reveal overlay (centered, similar to CardUpgradeRevealOverlay). Previously showed a generic joker animation with no actual card added. |
...
| Component | Purpose |
|-----------|---------|
| ~~`CardRewardScreen.svelte`~~ | **Deleted 2026-04-12.** Was the Svelte fallback card reward overlay. The active reward path is `rewardRoom` (Phaser `RewardRoomScene`). The `cardReward` screen type is kept in the type union for save compatibility only.
...
| `RewardRoomOverlay.svelte` | DOM accessibility overlay for the Reward Room Phaser scene. Mounted by CardApp.svelte when `currentScreen === 'rewardRoom'`. Contains 3 transparent DOM buttons overlaid on Phaser canvas buttons — keyboard-focusable, screen-reader-visible. **Continue** (`data-testid="btn-reward-room-continue"`) at ~50%/88% viewport; calls `triggerRewardRoomContinue()` (fully wired). **Relic Accept** (`btn-reward-room-relic-accept`) at ~50%/47%; calls `triggerRelicDetailAccept()` (emits `relicDetailAccept` on scene — awaits game-logic wiring in `showRelicDetail()`). **Relic Leave** (`btn-reward-room-relic-leave`) at ~50%/51%; calls `triggerRelicDetailLeave()`. Replaces the inline Continue button that was in CardApp.svelte (commit 003fe7d73). All dimensions use `calc(Npx * var(--layout-scale, 1))`. BATCH-ULTRA T11 — clears 4 lint:wiring WARNs on RewardRoomScene.ts:820,851,970,999. |
| `RunEndScreen.svelte` | Run summary: victory/defeat/retreat/abandon, XP breakdown, facts correct, floor reached. Props `result` accepts `'victory' | 'defeat' | 'retreat' | 'abandon'`. `computeGrade()` handles `'abandon'` with floor-based letter grades (same thresholds as defeat) and distinct title (`RUN ABANDONED`) and flavor text. Grade badge shown for abandon (only hidden for retreat). Calls `ambientAudio.setContext('run_end_victory')` or `setContext('run_end_defeat')` on `onMount` branched on `isVictory`. **Timer format:** `formatDuration(ms)` outputs `Nm 0Ss` (minutes first), e.g. `1m 01s`. Format verified correct (BATCH-2026-04-12-002 timer bug was pre-fixed). |
...
### TutorialCoachMark.svelte (added 2026-04-12, updated 2026-04-14)
...
`spotlight: boolean`, `blockInput?: boolean` (default false), `ondismiss: () => void`, `onskip: () => void`

**Positioning:** Queries `[data-tutorial-anchor="{target}"]` on mount + resize, uses `getBoundingClientRect()`.
For `target: 'enemy-sprite'` or `position: 'center'`: fixed at viewport ~25% from top, horizontally centered (updated from 35%).
Clamps to viewport edges (8px margin). Fallback tooltip size: 500×160px (updated from 320×120px).

**Visual:** Dark bg `rgba(6,8,16,0.92)`, gold border `rgba(241,196,15,0.5)`, CSS-triangle arrow pointing toward anchor.
z-index 960. Font `var(--font-rpg)` **28px** message, **22px** primary button, **18px** skip link (all doubled 2026-04-14).
Max-width `calc(500px * var(--layout-scale,1))` (updated from 320px). Padding 18/24px. Fade-in 200ms on mount.

**Spotlight:** When `spotlight=true`, renders `rgba(0,0,0,0.5)` overlay at z-index 959 with clip-path polygon
cutout (8px padding) around the anchor element. Normally `pointer-events: none` — gameplay is not blocked.
When `blockInput=true`, spotlight gains `pointer-events: all` to block game interaction.

**Input Blocking:** When `blockInput=true` and `spotlight=false`, renders a separate `.tutorial-block-overlay`
(fixed, full-screen, z-index 959, `pointer-events: all`) that prevents all game input until dismissed.
Driven by `tutorialBlocksInput` writable store in `tutorialService.ts`.
...
- `enemy-intent` — `.enemy-intent-bubble` div (added 2026-04-14)
- `enemy-power-badges` — wrapper div around `<EnemyPowerBadges>` (added 2026-04-14)
- `draw-pile` — `.draw-pile-indicator` div (added 2026-04-14)
- `discard-pile` — `.discard-pile-indicator` div (added 2026-04-14)

**data-tutorial-anchor attributes in InRunTopBar.svelte (added 2026-04-14):**
- `gold-counter` — `.gold-counter` div
- `relics-row` — `.relics-row` div
- `deck-btn` — deck viewer `<button>`
- `pause-btn` — pause gear `<button>`
- `fog-wing-wrapper` — `.fog-wing-wrapper` div (only present when fogState is defined)

**data-tutorial-anchor attributes in MusicWidget.svelte (added 2026-04-14):**
- `music-widget` — outermost `.music-widget` div
...
**Service:** Driven by `tutorialService.ts` stores (`tutorialActive`, `tutorialMessage`, `tutorialAnchor`, `tutorialSpotlight`, `tutorialBlocksInput`).
Calls `advanceStep()` on dismiss, `skipTutorial()` on skip.
`blockInput` prop in CardApp is wired to `$tutorialBlocksInput` store.
...
- Abandon button (`.banner-abandon-btn`): muted slate background, `min-height: 44px`. **Label change (2026-04-14):** Button text changed from "Abandon & Start New" to "Abandon Run" — now only abandons the active run and returns to the hub, rather than immediately starting a new run.
...
- **State**: `selectedIndex: number` (which run row is active), `filterResult: 'all' | 'victory' | 'defeat' | 'retreat' | 'abandon'`
...
- **Expedition Record tiles**: 5 tiles (Total Runs, Victories, Defeats, Retreats, Abandoned) in a `repeat(5, 1fr)` grid. `totalAbandons` derived from `stats?.totalAbandons ?? 0`.
- **Scaling**: Same rules as JournalScreen. Landscape tile grids expand from 2-column to 5-column for record tiles, 4-column for mastery/streak tiles.
