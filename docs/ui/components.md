...
- Badge color: green when free, red when cost > 1, default otherwise
...
| `CardPickerOverlay.svelte` | Pick a specific card from the deck (e.g., transmute target). **Ch15.1 (2026-04-08):** Enhanced with scrollable CSS grid layout (replaces fixed 3-card flex row), `mode` prop (`single | multi | multiUpTo`), and `confirmLabel` prop. **Props:** `title`, `cards`, `onselect`, `onskip`, `pickCount?` (default 1, backward-compatible), `mode?` (default: inferred from pickCount — `single` when pickCount=1, `multi` otherwise), `confirmLabel?` (default `"Confirm"`). **Mode behavior:** `single` = click auto-confirms (no confirm button); `multi` = must select exactly `pickCount` cards then click Confirm (enabled only when all picks made); `multiUpTo` = select 1..pickCount, Confirm enabled when ≥1 selected. **Grid layout:** `.picker-cards` uses `display: grid; grid-template-columns: repeat(auto-fill, minmax(calc(160px * var(--layout-scale, 1)), 1fr)); overflow-y: auto; max-height: calc(70vh)` — handles 3 cards (combat mechanics) and 20+ cards (deck-wide selection) identically. Container `max-width` 960px (scaled). Scrollbar styled gold-tinted thin. **Confirm button:** disabled/dim when criteria unmet, gold when enabled. **Backward compatibility:** callers that only pass `pickCount` + `onselect` retain identical behavior — no breaking changes. **Multi-mode onselect note:** In `multi` mode, `handleConfirm()` calls `onselect(card)` once per selected card (not as an array). Callers that need to batch selections (e.g. study upgrade in `CardApp.svelte`) must accumulate picks in a buffer — see `handleStudyUpgradeSelect()` / `handleStudyUpgradeSkip()`. **CardApp wiring (2026-04-08, tasks 15.2b + 14.3b):** `CardApp.svelte` now subscribes to `pendingStudyUpgrade` (study upgrade picker, multi mode) and `pendingTransformOptions` (shop transform picker, single mode) from `gameFlowController.ts`. Both render `CardPickerOverlay` when their store is non-null. **Phase 3 polish (2026-04-09):** Card render pipeline unified with `CardHand.svelte` — candidates now render using the same V2 layered frame (border, art, base, banner, upgrade icon), GUIDE_STYLES absolute positioning, and full `getCardDescriptionParts()` + `groupIntoLines()` rich text rendering. Description parts (`desc-number`, `desc-keyword`, `desc-mastery-bonus`, `desc-conditional`) styled identically to hand cards. Mechanic name rendered inside frame via `.v2-mechanic-name`. Chain color pill via `.v2-card-type`. AP cost gem via `.v2-ap-cost`. Effect text size class (`effect-text-md/sm/xs`) based on description length. Card label below frame shows mechanic name in smaller text. The old bespoke `.card-v2-frame` / `.card-info` layout (name + short description text below frame) has been replaced. **Post-pick card-hang fix (2026-04-09):** `CardCombatOverlay.svelte` now removes the source card from `animatingCards` and calls `resetCardFlow()` immediately when `pendingCardPick` is returned from the resolver — the card was being added to `animatingCards` before the resolver result was known, causing it to float mid-screen while the picker was open. **Dev store (2026-04-09):** `debugBridge.ts` now registers `pendingTransformOptions` at `Symbol.for("rr:pendingTransformOptions")` for visual testing via eval injection. **Phase 3 regression hotfix (2026-04-09):** Fixed 6 CSS discrepancies between the picker and CardHand that caused broken rendering: (1) `.frame-layer` `object-fit` changed from `fill` to `contain` — `fill` stretched frame images causing empty pentagon frames; (2) `.v2-mechanic-name` corrected to `color: #1a0a00`, `text-transform: uppercase`, `letter-spacing: 0.05em`, `white-space: nowrap`, `overflow: visible` — was using wrong light color and clipping name text; (3) `.v2-ap-cost` corrected to `color: #ffffff` with `-webkit-text-stroke: 1.5px #000` and correct text-shadow — was using gold color without outline; (4) `.frame-text` corrected to include `z-index: 5` — text overlays were below frame images; (5) `.mastery-glow` corrected to full hue-rotate+saturate filter (verbatim from CardHand) — was using simplified drop-shadow; (6) added `use:stretchText` on mechanic name div and imported the action. **Centering fix:** `.picker-cards` changed from `display: grid; grid-template-columns: repeat(auto-fill, minmax(..., 1fr))` to `display: flex; flex-wrap: wrap; justify-content: center` — flex wrapping properly centers 3 cards horizontally while still supporting 20+ card grids with wrapping. CSS leakage hypothesis was ruled out — Svelte scoped CSS cannot affect sibling component elements; hand card regression was caused by the same `object-fit: fill` issue on any picker-adjacent state, not true CSS leakage. |
