...
| `RunDeckOverlay.svelte` | **Full-run deck viewer (2026-04-11 rework).** Modal overlay showing ALL cards in the run as a responsive `CardVisual` grid (Slay-the-Spire style). **Grid layout:** `display: grid; grid-template-columns: repeat(auto-fill, minmax(calc(170px * var(--layout-scale, 1)), 1fr))` — auto-fills the panel width with ~170px card columns. Each card renders inside a `.rdo-card-visual-wrapper` (position:relative, sized for CardVisual's `inset:0` contract) with `--card-w` CSS var set for correct CardVisual typography scaling. Cards are wrapped in `.rdo-card-cell` (position:relative) which holds the CardVisual wrapper plus two absolute-positioned badges: `.rdo-card-pile-badge` (top-left corner, color-coded per pile) and `.rdo-card-mastery-badge` (bottom-right, shown only when masteryLevel > 0). **Sort/filter controls:** A `.rdo-controls` toolbar above the grid offers Filter (All / Hand / Draw / Discard / Forget) and Sort (Pile / Type / A-Z / Mastery) buttons. Filter drives `filterPile` state; sort drives `sortKey` state. `sortedFilteredCards()` derived re-sorts the full card list reactively. **Panel width:** `calc(900px * var(--layout-scale, 1))` (widened from 480px to fit the grid). Opened via the deck icon in `InRunTopBar`. Reads `$activeTurnState.deck`. Empty state shows human-readable message + escape button (softlock prevention). `data-testid="run-deck-overlay"`. Visibility driven by `runDeckOverlayOpen` writable store. Mounted in `CardApp.svelte` gated on `showTopBar && $runDeckOverlayOpen`. Close via backdrop click, close button, or Escape. z-index: 400. |
