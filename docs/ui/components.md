...
| `CardPickerOverlay.svelte` | Pick a specific card from the deck (e.g., transmute target). **Ch15.1 (2026-04-08):** Enhanced with scrollable CSS grid layout (replaces fixed 3-card flex row), `mode` prop (`single | multi | multiUpTo`), and `confirmLabel` prop. **Props:** `title`, `cards`, `onselect`, `onskip`, `pickCount?` (default 1, backward-compatible), `mode?` (default: inferred from pickCount — `single` when pickCount=1, `multi` otherwise), `confirmLabel?` (default `"Confirm"`). **Mode behavior:** `single` = click auto-confirms (no confirm button); `multi` = must select exactly `pickCount` cards then click Confirm (enabled only when all picks made); `multiUpTo` = select 1..pickCount, Confirm enabled when ≥1 selected. **Grid layout:** `.picker-cards` uses `display: grid; grid-template-columns: repeat(auto-fill, minmax(calc(160px * var(--layout-scale, 1)), 1fr)); overflow-y: auto; max-height: calc(60vh)` — handles 3 cards (combat mechanics) and 20+ cards (deck-wide selection) identically. Container `max-width` widened from 780px to 960px (scaled). Scrollbar styled gold-tinted thin. **Confirm button:** disabled/dim when criteria unmet, gold when enabled. **Backward compatibility:** callers that only pass `pickCount` + `onselect` retain identical behavior — no breaking changes. **Multi-mode onselect note:** In `multi` mode, `handleConfirm()` calls `onselect(card)` once per selected card (not as an array). Callers that need to batch selections (e.g. study upgrade in `CardApp.svelte`) must accumulate picks in a buffer — see `handleStudyUpgradeSelect()` / `handleStudyUpgradeSkip()`. **CardApp wiring (2026-04-08, tasks 15.2b + 14.3b):** `CardApp.svelte` now subscribes to `pendingStudyUpgrade` (study upgrade picker, multi mode) and `pendingTransformOptions` (shop transform picker, single mode) from `gameFlowController.ts`. Both render `CardPickerOverlay` when their store is non-null. |
