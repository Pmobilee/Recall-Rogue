...
| Component | Purpose |
|-----------|---------|
| `ShopRoomOverlay.svelte` | Shop: buy/sell cards and relics, card removal, card transform, haggle quiz. Calls `ambientAudio.setContext('shop')` on `$effect`. **Visual overhaul (2026-04-13):** Cards now render using `CardVisual.svelte` (pixel-perfect V2 layered frame, 170px wide at scale 1, `--card-w` CSS var required) in a flex-wrap `.cards-grid`. Relics now render as `.relic-float-card` articles: 64px circular icon (`relic-icon-circle`) with rarity-colored glow border, `getRelicIconPath(id)` image, `@keyframes relic-float` bob animation (0%→50%→100% translateY -6px), name + rarity pill + description always visible inline (no tooltip-only hiding). Services section: **Card Transform wired** — `service-transform` article reads `shopInventory.transformCost`, opens transform picker modal, calls `ontransform(cardId, haggled)` prop → `gameFlowController.onShopTransform()` → populates `pendingTransformOptions` store → `showTransformOptions` overlay renders `CardVisual` grid of 3 replacement choices, `onShopTransformChoice(card)` called on pick. Old `service-unaffordable` pattern replaced: services dim with `opacity: 0.6` instead of old `opacity: 0.5; pointer-events: none`. **HUD black-bar fix (2026-04-13):** `.shop-hud` uses `margin: 0 calc(-16px * var(--layout-scale, 1))` negative horizontal margins to extend full viewport width and prevent gap artifacts on the right edge. **Settings gear added (2026-04-13):** `.hud-settings-btn` with gear SVG (matches InRunTopBar `.pause-btn` pattern) added to right side of HUD. **Props added:** `ontransform?: (cardId: string, haggled: boolean) => void` and `ontransformchoice?: (card: Card) => void`. **ShopInventory.transformCost** now read from `shopInventory` prop. **Relic slot cap feedback (2026-04-12):** When `isRelicSlotsFull(runRelics)` is `true`, RELICS section label shows red badge, buy buttons disabled. **Relic tooltip (2026-04-12):** `.tooltip-backdrop` is `pointer-events: none`; click-outside dismiss via `document pointerdown` `$effect`. **Leave-shop confirm dialog (2026-04-12):** Inline modal replaces `window.confirm()` (blocked in headless Chrome). **Haggle fail behavior (2026-04-08):** No price penalty on fail — item keeps original price, haggle button disabled. |
