...
| `KnowledgeLibrary.svelte` | Full-screen knowledge browser (screen: `library`). Props: `onback: () => void`. No tab switcher — renders domain grid, domain fact list, and fact detail views directly. **Domain grid:** `repeat(auto-fill, minmax(240px, 1fr))` with landscape override to 4 columns. Each domain card has a left accent border via `--domain-accent` CSS variable, hover lift + box-shadow, and a domain icon from `getDomainMetadata().icon`. Section heading `Knowledge Domains` (15px, `#94a3b8`) renders above the grid. Deck Builder tab removed 2026-04-06. **Domain detail view (2026-04-06):** shows a `.domain-header` row (icon 24px + display name h3 18px + fact count badge) with a bottom border separator above the subcategory bar. `.sub-chip` visibility improved: font-size 12px (was 11px), padding 6×14px (was 4×10px), border `rgba(148,163,184,0.5)` (was 0.3), text `#cbd5e1` (was `#94a3b8`), hover state darkens background; `.sub-chip.active` is `rgba(59,130,246,0.4)` bg + `#60a5fa` border + `#f1f5f9` text + font-weight 600. **Search bar** (`.search-bar`) renders between the filters row and the fact list: `.search-input` is full-width, 40px min-height, 13px font, dark glass background, focus border `#3b82f6`; clear button (`.search-clear`) appears at right when query is non-empty; `.search-count` shows "N of M facts" below the bar when filtering. State: `searchQuery` (`()`) drives `filteredDomainEntries` (``) — filters on `statement`, `correctAnswer`, and `quizQuestion`. `searchQuery` resets to `''` when navigating back from domain view (back button onclick) or pressing Escape. `.library-topbar` (sticky header) background changed to fully opaque `#081225` (was `linear-gradient(180deg, #081225, rgba(8, 18, 37, 0.9))`) to prevent Phaser scene bleed-through (2026-04-06). |
