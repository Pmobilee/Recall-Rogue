...
| `NarrativeOverlay.svelte` | Full-screen atmospheric narrative overlay for room transitions and NPC dialogue. Two modes: **auto-fade** (all lines shown at once, auto-dismiss after 3-4s, click skips) and **click-through** (one line at a time, click advances). Black background with CSS fog animation, italic RPG-styled text, subtle hint prompt. Props: `lines: NarrativeLine[]`, `mode: 'auto-fade' | 'click-through'`, `onDismiss: () => void`. z-index 950. Driven by `narrativeStore.ts` (`showNarrative()` / `dismissNarrative()`). **Wired into `CardApp.svelte`** — gated by `$narrativeDisplay.active` (NOT by `$currentScreen`), renders above other overlays at z-index 950, below screen-transition at z-index 10000. See docs/mechanics/narrative.md §Display System. |
