...
| Component | Purpose |
|-----------|---------|
| `CardRewardScreen.svelte` | Post-combat reward: animated gold/heal reveal then 3-card pick. **Landscape sizing (2026-04-01):** `.altar-shell` width `min(80vw, 1200px)`. `.altar-option` min-height 300px, padding 54/15/18px. `.altar-options` gap 24px. `.mini-card-name` 20px. `.mini-card-desc` 16px. **Title position (Issue 4, 2026-04-11):** `.altar-header h1` gets `transform: translateY(calc(-8px * var(--layout-scale, 1)))` to pull the "Choose a Card" title flush against the decorative banner top edge. **Landscape scale fix (2026-04-12):** Removed `transform: scale(1.5)` from `.altar-shell` and `.actions` in landscape mode — `--layout-scale` is already 1.5 at 1920×1080 so the additional CSS scale was doubling it, pushing the header above the viewport (y=−175 in playtests). |
