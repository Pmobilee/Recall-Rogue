...
| Component | Purpose |
|-----------|---------|
| `CardRewardScreen.svelte` | Post-combat reward: animated gold/heal reveal then 3-card pick. **Landscape sizing (2026-04-01):** `.altar-shell` width `min(80vw, 1200px)`, scale 1.5 (was 65vw/900px/1.15). `.altar-option` min-height 300px (was 200px), padding 54/15/18px (was 36/10/12px). `.altar-options` gap 24px (was 16px). `.mini-card-name` 20px (was 15px). `.mini-card-desc` 16px (was 12px). **Title position (Issue 4, 2026-04-11):** `.altar-header h1` gets `transform: translateY(calc(-8px * var(--layout-scale, 1)))` to pull the "Choose a Card" title flush against the decorative banner top edge. |
