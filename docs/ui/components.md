...
- **Typing input (2026-04-13):** `.typing-input` padding: `14/16px` (was `10/12px`), font-size: `22px` (was `16px`). `.typing-submit-btn` padding: `14/20px`, font-size: `22px`. Applied in both `GrammarTypingInput.svelte` and `TypingInput.svelte`.
...
| Component | Purpose |
|-----------|---------|
| `ShopRoomOverlay.svelte` | Shop: buy/sell cards and relics, card removal, haggle quiz. Calls `ambientAudio.setContext('shop')` on `$effect`. **Relic slot cap feedback (2026-04-12):** When `isRelicSlotsFull(runRelics)` is `true`, the RELICS section label shows a red `Relic slots full` badge, relic buy buttons are disabled (show `Full` instead of price), and the `<article>` gains `.slots-full` (grayscale+opacity). Reactivity via `$effect` subscribing to `activeRunState`. Import: `isRelicSlotsFull` from `relicEffectResolver.ts`. **Layout (2026-04-08):** `.shop-hud` height `48px`, `background: rgba(10, 15, 25, 0.95)`, `border-bottom: 2px solid rgba(194, 157, 72, 0.5)` — matches InRunTopBar gold style. `.unaffordable` uses `filter: grayscale(0.3)` + `border-color: rgba(239, 68, 68, 0.3)` instead of `opacity: 0.4` (keeps text readable). **Synergy system removed (2026-04-08):** `highlightedMechanics` state, `onCardHover`/`onCardLeave` handlers, `deckMechanics` derived, synergy-badge template, `.synergy-match`, `.synergy-none`, `.synergy-highlight`, and `synergy-pulse` keyframes all deleted. **Card display (2026-04-08):** Shop card items now show a `.chain-dot` colored pip (8px circle) inline with the effect label via `.card-sub-row` flex row — indicates chain type without synergy cross-highlighting. **Relic tooltip (2026-04-12 — PATCHED):** Hover triggers `showRelicTooltip()`, mouseleave triggers `dismissRelicTooltip()`. Shows relic name (rarity-colored), description, and trigger; max-width `220px` scaled, z-index 200, dark background. **CRITICAL — tooltip-backdrop must be `pointer-events: none`:** The `.tooltip-backdrop` element (z-index 199, `position: fixed; inset: 0`) was originally a `<button>` with `pointer-events: auto` to catch click-outside dismissals. This ate every click in the viewport while the tooltip was showing — buy button clicks went to the backdrop instead of opening the purchase modal (BATCH-2026-04-12-001-C-004). Fix (2026-04-12): backdrop changed to `<div aria-hidden>` with `pointer-events: none`; click-outside dismiss now handled by a `$effect` that adds a `document pointerdown` listener (capture phase) when `relicTooltip` is non-null, clearing it on any click outside `.relic-tooltip`. Mouse-away dismiss (`onmouseleave`) still works as before. **Leave-shop confirm dialog (2026-04-12 — Bug A):** `handleLeaveShop()` previously called `window.confirm()` which returns `false` silently in headless Chrome/automated contexts, making shop exit unreachable in playtests. Fix: replaced with an inline Svelte confirm modal driven by `showLeaveConfirm = $state(false)`. When the player has affordable items remaining, clicking `btn-leave-shop` sets `showLeaveConfirm = true`, rendering a `.leave-confirm-modal` with a red "Leave anyway" button (`data-testid="btn-leave-confirm"`) and a grey "Stay" button (`data-testid="btn-leave-cancel"`). `confirmLeave()` calls `showBark('leave_with_gold')`, `playCardAudio('shop-close')`, and `ondone()`. If no affordable items remain, shop exits immediately without the dialog. The modal uses `.modal-backdrop` (full-screen dim overlay with `role="dialog" aria-modal="true"`) — consistent with the relic-removal picker pattern. **Haggle fail behavior (2026-04-08):** On haggle quiz failure, NO price penalty is applied — the item keeps its original price and the haggle button is disabled for that item (`haggledThisItem = true`). The `penaltyPrice` state variable and all related markup/computation have been removed. |
