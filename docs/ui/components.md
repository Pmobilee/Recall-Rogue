...
| `PlaylistBar.svelte` | Horizontal bar showing the active custom deck. Fallback name shows "Custom Deck" when no named playlist exists. **Meta display (2026-04-07):** shows deck names instead of item count — up to 3 names joined with commas; 3+ collapses to first two names + "+N more". **View button** wires to `onViewPlaylist` callback (previously no-op, now opens `PlaylistViewModal`). |
...
| `PlaylistViewModal.svelte` | Modal for viewing and editing a custom playlist. **Props:** `playlist: CustomPlaylist`, `onClose`, `onRemoveItem(itemIndex: number)`, `onDeletePlaylist`, `onRenamePlaylist(newName: string)`. **Features:** inline name editing (click playlist name to enter edit mode, Enter/blur to commit), per-item remove buttons (X), total-facts footer summary, delete-with-confirmation flow ("Delete Playlist" → inline confirm/cancel). Deck display names resolved from the registry (`getAllDecks()`); fact counts from `getCuratedDeckFacts()`. Styled with dark semi-transparent backdrop (z-index 350), 460px panel, all sizing uses `calc(Npx * var(--layout-scale, 1))` / `calc(Npx * var(--text-scale, 1))`. Added 2026-04-07. |
