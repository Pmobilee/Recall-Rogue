...
| `PlaylistViewModal.svelte` | Rich inspection modal for viewing and editing a custom playlist (upgraded 2026-04-07). **Props:** `playlist: CustomPlaylist`, `onClose`, `onRemoveItem(itemIndex: number)`, `onDeletePlaylist`, `onRenamePlaylist(newName: string)`. **Features:** (1) **Aggregate stats header** — overall progress bar (green fill via `.progress-track`/`.progress-fill`), total/seen/mastered counts; uses `getDeckProgress()` summed across all study items. (2) **Expandable accordion items** — each deck row shows name, mini 80px progress bar, mastery %, fact count, and a chevron (▸/▾) if the deck has sub-decks or exam tags; clicking expands the row using a proper `<button class="expand-trigger">` (a11y-safe, aria-expanded). (3) **Sub-deck drill-down** (in expanded panel) — lists each sub-deck with its own mini progress bar and mastery % via `getSubDeckProgress()`. (4) **Exam tag chips** (in expanded panel) — shows tag display labels (using `formatExamTag()` from `src/ui/utils/examTagDisplay.ts`) with fact counts via `getTagFactIds()`. Tags are display-only (not toggleable in this context). **Inline name editing**, per-item remove buttons, delete-with-confirmation flow all preserved. Modal width widened from 460px to **560px** to accommodate progress bars. Backdrop z-index 350. All sizing uses `calc(Npx * var(--layout-scale, 1))` / `calc(Npx * var(--text-scale, 1))`. |
