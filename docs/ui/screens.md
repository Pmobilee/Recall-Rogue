...
| `rewardRoom` | *(Phaser scene)* | `RewardRoomScene` handles rendering; `RewardCardDetail` as DOM overlay. **A11y (BATCH-ULTRA T11):** `RewardRoomOverlay.svelte` renders 3 transparent DOM buttons over the Phaser canvas: Continue (`btn-reward-room-continue`, fully wired to `triggerRewardRoomContinue()`), Relic Accept (`btn-reward-room-relic-accept`, wired via `triggerRelicDetailAccept()` — scene-side event handler pending game-logic wiring), Relic Leave (`btn-reward-room-relic-leave`, same). Clears all 4 lint:wiring WARNs for `RewardRoomScene.ts`. |
