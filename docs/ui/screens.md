...
| `multiplayerMenu` | Multiplayer entry screen — three-tab UI (Create Lobby with 5 mode cards + visibility picker, Join Lobby with 6-char code, LAN Play); shown when hub "Multiplayer" button is pressed; entry point before lobby creation |
...
| `comingSoon` | Multiplayer coming-soon placeholder — shown when the tent is clicked with `MULTIPLAYER_ENABLED=false`. Title + 2-line body + single Back button returning to hub. Not gated on the flag — it IS the gate. **Added 2026-04-26.** |
...
| `shopRoom` | `ShopRoomOverlay` | **Layout (2026-04-14):** Side-by-side two-column layout — relics left (~33%, `.relics-panel`, transparent float, no container bg), cards right (~66%, `.cards-area`). Relic name 20px, description 22px. Service button labels/prices 21px. |
...
| `comingSoon` | `ComingSoonScreen` | Props: `onBack` (→ `hub`). Not gated on `MULTIPLAYER_ENABLED` — renders unconditionally when screen is `comingSoon`. **Added 2026-04-26.** |
| `multiplayerMenu` | `MultiplayerMenu` | Two-tab entry screen. Create tab: 5 mode cards (race/same_cards/duel/coop/trivia_night) + "Create Lobby" button. Join tab: monospace 6-char code input + "Join Lobby" button. **Browse Lobbies button (2026-04-11):** gold button in the Create tab footer calls `onBrowseLobbies` → transitions to `lobbyBrowser`. Props: `onBack`, `onCreateLobby(mode, opts: { visibility, password? })`, `onJoinLobby(code)`, `onBrowseLobbies`. **Visibility picker (2026-04-20):** Radio pills below mode list — Public / Password / Friends Only. Password pill shows input field; validates on create. Friends Only disabled when not Steam. `onBack` returns to hub; creating transitions to `multiplayerLobby` with new lobby; joining transitions to `multiplayerLobby` (lobby syncs via onLobbyUpdate). **Error banner (2026-04-20):** `handleCreateLobby` and `handleJoinLobby` in `CardApp.svelte` are wrapped in try/catch; on failure they set `multiplayerError` (shown as `.mp-error-banner` at top of screen) and `console.error('[CardApp] createLobby/joinLobby failed:', error)`. Error clears on dismiss, back, tab switch, or re-attempt. **No-scroll layout (2026-04-22):** Create tab fits 1920×1080 without scrolling. Mode list uses 2-column grid (5 cards → 3 rows); `.mode-desc` hidden by default, shown only on the selected card. Visibility pills + lobby name input are in a flex row (`.create-row`). `.mp-body` is `overflow: hidden`. |
...
| `multiplayerLobby` | `MultiplayerLobby` | Only mounts when `currentLobby !== null`; Props: `lobby`, `localPlayerId`, `onBack`. `MultiplayerHUD` also overlaid during `combat` when `isMultiplayerRun` is true. **Ready-timeout banner (H-008, 2026-04-23):** Subscribes to `onReadyTimeout` from `multiplayerLobbyService`. When the 30s watchdog fires (host never sent `mp:lobby:start`), shows a full-width error banner above the three-column body: "Host didn't start the match. You can ready up again or leave." with Leave lobby and Try again buttons. `readyPending` is reset in the handler. Live countdown shown in the Ready button label while waiting (1s poll via `getReadyWatchdogStatus()`). Subscription cleaned up in `onDestroy`. |
| `settings` | `SettingsPanel` | Four-tab settings (Audio, Accessibility, Notifications, Account). **Resolution dropdown (2026-04-22):** In the Accessibility tab, immediately after the Fullscreen toggle, a `select-window-resolution` dropdown lists five 16:9 presets (1280×720 through 3840×2160). Hidden on mobile and when fullscreen is active. Non-standard window sizes show a disabled "Custom (W × H)" entry as the preselected value. Selection calls `setWindowResolution()` from `fullscreenService.ts` and persists the choice to `localStorage` under key `rr-window-resolution`. On app boot, `CardApp.svelte` reads this key and calls `setWindowResolution()` to restore the saved size. |
...
> **Steam v1.0 launch note:** Multiplayer is flag-gated OFF for the initial release. `MULTIPLAYER_ENABLED = false` in `src/config/featureFlags.ts`. The tent button is **visible** but clicks route to the `comingSoon` screen (a placeholder explaining MP modes ship post-launch). All five live MP screens (`multiplayerMenu`, `lobbyBrowser`, `multiplayerLobby`, `triviaRound`, `raceResults`) remain blocked from rendering. `?mp` BroadcastChannel dev mode is a no-op. Flip the flag to `true` and rebuild to re-enable. See `docs/architecture/multiplayer.md` → Steam v1.0 Launch — Flag Gate.
...
- Leaves to: `dungeonMap` (via "Begin Expedition"), `studyTemple` (via "Back" — resets `shuffleSeedOffset` then calls `abandonActiveRun('studyTemple')` which handles teardown and transition atomically), or reshuffles in-place (via "Shuffle Chains")

### Props

| Prop | Type | Description |
|------|------|-------------|
| `onBack` | `() => void` | Called when "Back" is clicked. Resets `shuffleSeedOffset` to 0 then calls `abandonActiveRun('studyTemple')`. |
| `onShuffle` | `() => void` | Called when "Shuffle Chains" is clicked. Calls `reshuffleChainDistribution(++shuffleSeedOffset)`. |
| `onBeginExpedition` | `() => void` | Called when "Begin Expedition" is clicked. Calls `confirmChainDistribution()` then resets `shuffleSeedOffset` to 0. |
...
- `handleRunPreviewBack()` — resets `shuffleSeedOffset = 0`, then calls `abandonActiveRun('studyTemple')`. Passing the target screen directly eliminates the hub-flash that occurred when `abandonActiveRun()` set `currentScreen='hub'` before a subsequent `transitionScreen` override.
- `handleRunPreviewBeginExpedition()` — calls `confirmChainDistribution()` (imported from `gameFlowController`), then resets `shuffleSeedOffset = 0` so the offset doesn't accumulate across runs
