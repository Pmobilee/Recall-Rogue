...
> **Last verified:** 2026-04-22 (Added MpDebugOverlay for MP live diagnostics. Added forensic contentSelection logging in MultiplayerLobby. Updated onLobbyUpdate to explicitly spread contentSelection for Svelte reactivity. Also: error banner, visibility picker, PlayerRosterPanel.)
...
| `MpDebugOverlay.svelte` | **Developer-only live multiplayer diagnostics overlay (2026-04-22).** Gated on `$devMode || mpLobbyActive` — shows when devMode is on OR when the player is in a multiplayer lobby (BUG9, 2026-04-22d). **Activation methods:** (1) `?dev=true` URL param, (2) `VITE_DEV_TOOLS=1` build env var, (3) `localStorage.rr-dev-mode=1` (persisted toggle), (4) `Cmd+Shift+D` / `Ctrl+Shift+D` keyboard chord (runtime toggle — works in Tauri release builds where URL params are inaccessible), (5) automatic when in any multiplayer lobby. Renders nothing in production / normal solo gameplay. **Mount point:** Top-level in `CardApp.svelte`, outside scene containers — survives screen transitions. **Position:** Fixed top-right corner, `z-index: 9999`, semi-transparent dark background, monospace font, dynamic scaling throughout. **Refresh:** `setInterval` at 1 s in a `$effect` with cleanup; reads `window.__rrMpState` via `getMpDebugState()` from `src/services/mpDebugState.ts`. **Sections:** Lobby (id, code, mode, deckSelectionMode, playerCount, contentSelection.type, contentSelection.decks.length), Transport (state, peerId, preConnectBufferSize, lastSendResult, lastError), Steam/P2P (localSteamId, localPlayerId, live p2pConnectionState polled via `steam_get_p2p_connection_state` / `getP2PConnectionState` — shows "(cmd unavailable)" if not yet wired), LAN (boundUrl, lastProbeResult, lastError). **Probe tool:** Host + port inputs + "Probe" button calling `lanTcpProbe` (gracefully degrades). **Copy button:** Writes `JSON.stringify(window.__rrMpState)` to clipboard. **Minimize toggle:** Collapses to header only. All text is developer-facing only — exempt from humanizer rule. All sizing via `calc(Npx * var(--layout-scale/--text-scale, 1))`. |
