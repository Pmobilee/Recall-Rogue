...
**`powerOverride` must be pre-multiplied (2026-04-18 fix):** `getCardDescriptionParts()` accepts an optional `powerOverride` for in-hand display (the card face CC value from `computeDamagePreview`). This value already has chain multiplier baked in. The chain-multiplication block in `getCardDescriptionParts()` now skips when `powerOverride` is provided, preventing double-application. Without this guard, a chain-1.2x card showed `round(7 × 1.2) = 8` but applied 7. See `docs/gotchas.md` 2026-04-18 "Chain multiplier double-applied on card text display".
...
| Chain multiplier (`chainMultiplier`) | CC attack/shield base scales with active chain (CC-only; QP breaks the chain) |
| Enemy `chainVulnerable` | +50% CC attack damage when chain > 1.0 (attack-only; mirrors turnManager lines 1956–1963) |

**Modifiers EXCLUDED (unknowable at preview time):**

Speed bonus, factDamageBonus, crescendo streak, memory_palace streak.
...
**Strike curve (2026-05-13):** Strike progresses `4 → 5 → 5 → 6 → 7 → 8`
for Quick Play value from L0 to L5. L1 is intentionally a visible +1 so the
first Reading Nook-style upgrade does not show two identical card faces.
