...
export class CombatScene extends Phaser.Scene {
  ...
  /** When non-null, overrides the HP bar width/height scale during quiz slide (0.85 when active). */
  ...
  slideEnemyForQuiz(active: boolean): void {
    ...
    // Scale enemy to 0.85x during quiz — keeps enemy readable while leaving room for the quiz overlay
    const targetScale = active ? 0.85 : 1.0
  ...
  setEnemy(
    ...
    // GL-readiness guard lives in EnemySpriteSystem.setSprite: if the WebGL context
    // is null right after scene restart, setSprite defers itself via delayedCall(250ms).
    const texture = enemyTextureKey(this.currentEnemyId, 'idle')
    const hasSprite = hasTexture(this, texture)

    // GL-readiness guard is inside EnemySpriteSystem.setSprite — it defers itself
    // if the WebGL context is not ready after scene restart. No retry needed here.
    if (hasSprite) {
