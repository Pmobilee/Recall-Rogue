<script lang="ts">
  ...
  const EFFECT_INFO: Record<string, { name: string; icon: string; spriteIcon?: string; color: string; desc: (v: number, t: number) => string }> = {
    ...
    double_strike: { name: 'Double Strike', icon: '⚔️', spriteIcon: '/assets/sprites/icons/icon_reward_twin_blades.png', color: '#fb923c', desc: (v) => `Next attack hits twice at ${v}%` },
    focus: { name: 'Focus', icon: '🔮', spriteIcon: '/assets/sprites/icons/icon_reward_focus_crystal.png', color: '#c084fc', desc: (v) => `Next ${v} card${v !== 1 ? 's' : ''} cost 1 less AP` },
    foresight: { name: 'Foresight', icon: '👁️', spriteIcon: '/assets/sprites/icons/icon_reward_fortune_eye.png', color: '#67e8f9', desc: (v, t) => `See enemy intents (${t} turn${t !== 1 ? 's' : ''} left)` },
    fortify: { name: 'Fortify', icon: '🏰', spriteIcon: '/assets/sprites/icons/icon_reward_mirror_guard.png', color: '#94a3b8', desc: (v) => `${v} block persists into next turn` },
    overclock: { name: 'Overclock', icon: '⚙️', spriteIcon: '/assets/sprites/icons/icon_reward_tempo_charm.png', color: '#e879f9', desc: () => `Next card effect doubled, draw -1 next turn` },
    ...
    hardcover: { name: 'Hardcover', icon: '📖', spriteIcon: '/assets/sprites/icons/icon_reward_field_tome.png', color: '#a78bfa', desc: (v) => `Hardcover armor: ${v}. Reduces Quick Play damage. Charges strip it away.` },
