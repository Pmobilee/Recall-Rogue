<script lang="ts">
  ...
  const STATUS_EFFECT_INFO: Record<string, { name: string; icon: string; spriteIcon?: string; color: string; desc: (v: number, t: number) => string }> = {
    ...
    foresight:     { name: 'Foresight',     icon: '👁️', spriteIcon: '/assets/sprites/icons/icon_reward_fortune_eye.png', color: '#67e8f9', desc: (_, t) => `See enemy intents (${t} left)` },
    fortify:       { name: 'Fortify',       icon: '🏰', spriteIcon: '/assets/sprites/icons/icon_reward_mirror_guard.png', color: '#94a3b8', desc: (v) => `${v} block persists next turn` },
    overclock:     { name: 'Overclock',     icon: '⚙️', spriteIcon: '/assets/sprites/icons/icon_reward_tempo_charm.png', color: '#e879f9', desc: () => `Next card doubled, draw -1` },
