...
export const ASCENSION_LEVEL_RULES: AscensionLevelRule[] = [
  ...
  { level: 9, name: 'Combo Breaker', effect: 'Combo resets on turn end (not just on wrong answer)' },
  { level: 10, name: 'Endurance', effect: 'Retreat rewards require floor 12+' },
  { level: 11, name: 'Fading Light', effect: 'Encounter 2 per floor has -2s timer' },
  { level: 12, name: 'Relic Tax', effect: 'Boss relic choices reduced to 2' },
  { level: 13, name: 'Deep Knowledge', effect: 'Tier 1 cards use 4-option MCQ' },
  { level: 14, name: 'Glass Cannon', effect: 'Player max HP reduced to 70' },
...
export function getAscensionModifiers(level: number): AscensionModifiers {
  ...
  return {
    ...
    encounterTwoTimerPenaltySeconds: l >= 11 ? 2 : 0,
    starterDeckSizeOverride: l >= 18 ? 10 : l >= 5 ? 12 : null,
    preventFlee: l >= 6,
    preferCloseDistractors: l >= 7,
    miniBossBossTierAttacks: l >= 8,
    comboResetsOnTurnEnd: l >= 9,
    minRetreatFloorForRewards: l >= 10 ? 12 : null,
    relicCap: l >= 12 ? 2 : 3,
    tier1OptionCount: l >= 13 ? 4 : 3,
    playerMaxHpOverride: l >= 14 ? 70 : null,
