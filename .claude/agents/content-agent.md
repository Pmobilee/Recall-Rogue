...
5. **MANDATORY output verification** — for narrow ad-hoc fixes, sample 5-10 items from the batch and read them back. For deck creation, re-check passes, or major modifications, sample **50 facts stratified across difficulty levels, sub-decks, and answer pools** per `.claude/rules/deck-quality.md` § "50-Fact Sampling Protocol". Check for broken grammar, "this structure" placeholders jammed into wrong positions, corrupted fields, duplicates. NEVER trust batch output without sampling. This caught 262 broken rewrites that would have shipped, plus 3 placeholder leaks during the 2026-04-10 re-check that 20-sample draws had previously missed. **Additionally, after EVERY file write, run `git status` and `git diff` to confirm your changes actually landed on disk — sub-agents (you) have been caught returning fabricated success summaries with zero bytes written.**
